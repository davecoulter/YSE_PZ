...
from .table_utils import (
    FieldTransientTable,
    AdjustFieldTransientTable,
    RisingTransientFilter,
)
...
def select_yse_fields(request):
    ...
    all_yse_fields = (
        SurveyFieldMSB.objects.filter(survey_fields__obs_group__name="YSE")
        .distinct()
        .order_by("name")
    )
    active_yse_fields = (
    ...
    for a in all_yse_fields:
        if a.name in active_names:
            continue
        obs = (
            SurveyObservation.objects.filter(survey_field=a.survey_fields.all()[0])
            .filter(obs_mjd__isnull=False)
            .order_by("-obs_mjd")
        )
        if len(obs):
    ...
    for a in active_yse_fields:
        ...
        obs_mjd = np.sort(
            np.array([om for om in obs.values_list("obs_mjd", flat=True)])
        )
        if len(obs_mjd):
    ...
    all_yse_fields = SurveyField.objects.filter(obs_group__name="YSE").order_by(
        "field_id"
    )

    #    import pdb; pdb.set_trace()
    context = {
        "yse_field_data": yse_field_data,
...
def return_serialized_transients(request):
    ...
    if len(request.META["QUERY_STRING"].replace("q=", "")) >= 5:
        transients = Transient.objects.filter(
            Q(name__startswith=request.META["QUERY_STRING"].replace("q=", ""))
...
def yse_sky(request):
    ...
    if request.method == "POST" and "raCenter" in request.POST.keys():
        ...
        if (not fovQuery) or (fovQuery.latest("created").canvas_x_grid_size != 500):

            field_names = SurveyFieldMSB.objects.values_list("survey_fields__field_id")
            fields = SurveyField.objects.filter(field_id__in=field_names).filter(
                active=True
            )
    ...
    coordForm = yse_forms.CoordForm(
        initial={
            "raCenter": fov.raCenter,
...
FROM YSE_App_transient t WHERE YSE_App_transient.id = t.id"""
    transients = transients.annotate(
        days_since_disc=RawSQL(days_from_disc_query, ())
    ).order_by("-days_since_disc")

    # 	nearbytransientfilter = RisingTransientFilter(request.GET, queryset=nearby_transients,prefix='ysenearby')
    transientfilter = RisingTransientFilter(
        request.GET, queryset=transients, prefix="default"
    )
...
def yse_planning(request):
    ...
    # rising transients
    rising_transients = (
        rising_transient_queryset(ndays=7)
...
"""
    rising_transients = rising_transients.annotate(
        recent_mag=RawSQL(recent_mag_raw_query, ())
    )
...
FROM YSE_App_transient t WHERE YSE_App_transient.id = t.id"""
    ...
    risingtransientfilter = RisingTransientFilter(
...
"""
    good_transients = good_transients.annotate(
        recent_mag=RawSQL(recent_mag_raw_query, ())
    ).filter(~Q(tags__name="YSE"))
...
FROM YSE_App_transient t WHERE YSE_App_transient.id = t.id"""
    ...
    goodtransientfilter = RisingTransientFilter(
        request.GET, queryset=good_transients, prefix="ysegood"
    )
...
"""
    ...
    nearby_transients = nearby_transients.annotate(
        recent_mag=RawSQL(recent_mag_raw_query, ())
    ).filter(~Q(tags__name="YSE"))
...
FROM YSE_App_transient t WHERE YSE_App_transient.id = t.id"""
    ...
    nearbytransientfilter = RisingTransientFilter(
        request.GET, queryset=nearby_transients, prefix="ysenearby"
    )
    table_nearby = AdjustFieldTransientTable(
        nearbytransientfilter.qs, prefix="ysenearby"
    )
...
def yse_msb_change(request, field_to_drop, field_to_add, snid, ra_to_add, dec_to_add):
    ...
    add_dict = {
        ...
        "ra_cen": ra_to_add,
        "dec_cen": dec_to_add,
        "ztf_field_id": field_to_add.split(".")[0],
        "cadence": 3,
...
def yse_fields(request, ra_min_hour, ra_max_hour, min_mag):
    ...
    ztfsurveyfields = (
        SurveyField.objects.filter(
            ~Q(obs_group__name="ZTF")
            & Q(ra_cen__gt=float(ra_min_hour) * 360 / 24.0)
            & Q(ra_cen__lt=float(ra_max_hour) * 360 / 24.0)
        )
        .values_list("ztf_field_id")
        .distinct()
    )
    survey_tables = []
    for s in ztfsurveyfields:
        surveyfields = SurveyField.objects.filter(ztf_field_id=s[0])
        if not len(surveyfields):
            continue
...
"""
        ...
        qs_final = qs_final.annotate(
            min_mag=Min("transientphotometry__transientphotdata__mag")
        )
...
FROM YSE_App_transient t WHERE YSE_App_transient.id = t.id"""
        ...
        risingtransientfilter = RisingTransientFilter(
            request.GET, queryset=qs_final, prefix=str(s[0])
        )
