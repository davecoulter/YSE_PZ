...
class Followup(BaseModel):
    ...
    status = models.ForeignKey(
        FollowupStatus, on_delete=models.SET(get_sentinel_followupstatus)
    )

    # Optional
    too_resource = models.ForeignKey(
        ToOResource, null=True, blank=True, on_delete=models.SET_NULL
    )
    classical_resource = models.ForeignKey(
        ClassicalResource, null=True, blank=True, on_delete=models.SET_NULL
    )
    queued_resource = models.ForeignKey(
        QueuedResource, null=True, blank=True, on_delete=models.SET_NULL
    )
...
class TransientFollowup(Followup):
    ...
    def __str__(self):
        return "Transient Followup: [%s]; Valid: %s to %s" % (
            self.transient.name,
            self.valid_start.strftime("%m/%d/%Y"),
            self.valid_stop.strftime("%m/%d/%Y"),
        )
...
def execute_after_save(sender, instance, created, *args, **kwargs):
    ...
    if created:
        ...
        for u in usertelescopes:
            if (
                instance.classical_resource
                and u.telescope.name == instance.classical_resource.telescope.name
            ):
                SendFollowingNotice(
                    instance.id,
                    instance.transient.name,
                    instance.classical_resource.telescope,
                    u.profile,
                )
...
class HostFollowup(Followup):
    ...
    def __str__(self):
        return "Host Followup: [%s]; Valid: %s to %s" % (
