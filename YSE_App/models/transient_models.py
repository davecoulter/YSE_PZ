...
class Transient(BaseModel):
    ...
    status = models.ForeignKey(
        TransientStatus, models.SET(get_sentinel_transientstatus)
    )
    obs_group = models.ForeignKey(ObservationGroup, on_delete=models.CASCADE)

    # Optional
    non_detect_band = models.ForeignKey(
        PhotometricBand,
        related_name="+",
        null=True,
        blank=True,
    ...
    best_spec_class = models.ForeignKey(
        TransientClass,
        related_name="+",
        null=True,
        blank=True,
    ...
    best_spectrum = models.ForeignKey(
        "TransientSpectrum",
        related_name="+",
        null=True,
        blank=True,
    ...
    abs_mag_peak_band = models.ForeignKey(
        PhotometricBand,
        related_name="+",
        null=True,
        blank=True,
    ...
    antares_classification = models.ForeignKey(
        AntaresClassification, null=True, blank=True, on_delete=models.SET_NULL
    )
    internal_survey = models.ForeignKey(
        InternalSurvey, null=True, blank=True, on_delete=models.SET_NULL
    )
    ...
    def LikelyYSEField(self):
        ...
        dec_offset = cd.Angle(3.4 / 2.0, unit=u.deg)

        sf = SurveyField.objects.filter(~Q(obs_group__name="ZTF")).filter(
            (
                Q(ra_cen__gt=self.ra - ra_offset.degree)
        ...
        if len(sf):
            so = (
                SurveyObservation.objects.filter(survey_field__field_id=sf[0].field_id)
                .filter(obs_mjd__isnull=False)
                .order_by("-obs_mjd")
            )
            if len(so):
                time_since_last_obs = (
                    date_to_mjd(datetime.datetime.utcnow()) - so[0].obs_mjd
                )
            else:
    ...
    def nearest_ztf_field(self):
        ...
        # Define the tile offsets:
        ra_offset = cd.Angle(width_corr / 2.0, unit=u.deg)
        dec_offset = cd.Angle(6.9 / 2.0, unit=u.deg)

        sf = SurveyField.objects.filter(obs_group__name="ZTF").filter(
            (
                Q(ra_cen__gt=self.ra - ra_offset.degree)
                & Q(ra_cen__lt=self.ra + ra_offset.degree)
                & Q(dec_cen__gt=self.dec - dec_offset.degree)
    ...
    def nearest_ztf_field_sep(self):
        ...
        sf = (
            SurveyField.objects.filter(obs_group__name="ZTF")
            .filter(
                (
                    Q(ra_cen__gt=self.ra - ra_offset.degree)
                    & Q(ra_cen__lt=self.ra + ra_offset.degree)
    ...
    def nearest_yse_field_sep(self):
        ...
        sf = (
            SurveyField.objects.filter(~Q(obs_group__name="ZTF"))
            .filter(
                (
                    Q(ra_cen__gt=self.ra - ra_offset.degree)
                    & Q(ra_cen__lt=self.ra + ra_offset.degree)
    ...
    def recent_mag(self):
        ...
        phot_data_query = Q(photometry__id__in=phot_ids)
        recent_mag = (
            yse_models.TransientPhotData.objects.exclude(data_quality__isnull=False)
...
def execute_after_save(sender, instance, created, *args, **kwargs):
    ...
    if created:
        ...
        if tag_TESS and instance.disc_date:
            TESSFlag = tess_obs(
                instance.ra, instance.dec, date_to_mjd(instance.disc_date) + 2400000.5
            )
            if TESSFlag:
                try:
            ...
            TESSFlag = tess_obs(
                instance.ra,
                instance.dec,
                date_to_mjd(instance.modified_date) + 2400000.5,
            )
            if TESSFlag:
                try:
                    tesstag = TransientTag.objects.get(name="TESS")
...
class AlternateTransientNames(BaseModel):
    ...
    obs_group = models.ForeignKey(
        ObservationGroup, null=True, blank=True, on_delete=models.SET_NULL
    )
