...
class TargetFieldTable(tables.Table):
	...
	obsButton = tables.columns.TemplateColumn(template_name=button_template_name_file,
											  orderable=True,
											  verbose_name='Currently Observing')

	fieldLink = tables.columns.TemplateColumn("<a href=\" {% url 'msb_detail' record.id %} \"> {{ record.name }} </a>",
											  orderable=True,
											  verbose_name='Field')
	fieldSet = tables.Column(accessor='fieldSet',
	...
	def order_distance(self,QuerySet,is_descending):
		...
		if len(fov):
			...
			QuerySet = QuerySet.annotate(
				...
				(Avg('survey_fields__dec_cen')-decC)**2).order_by(('-' if is_descending else '') + 'offset')


			return (QuerySet,True)
		else: return (fov,True)
		
	def order_obsButton(self, QuerySet, is_descending):
		QuerySet = QuerySet.annotate(
			amount=F('obsStatus')).order_by(('-' if is_descending else '') + 'amount')
		return (QuerySet, True)
