...
def toggle_field(request):
	...
	if request.method == 'POST':
		try:
			...
			tfs = SurveyFieldMSB.objects.get(id__exact=targetField_id).survey_fields.select_related()
...
def toggle_fieldset(request):
	...
	if request.method == 'POST':
		try:
			...
			return JsonResponse({'newState':fs.fieldSetActive}) # this must be returned for toggling
...
def get_field_verts(request):
	...
	if request.method == 'POST':
		...
		try:
			...
			w.wcs.crpix = [fov.canvas_x_grid_size/2.,
						   fov.canvas_y_grid_size/2.]
			# pixel deltas (north up, east left, pixel origin at upper left)
			w.wcs.cdelt = np.array([-1.*fov.fovWidth/fov.canvas_x_grid_size, 
									-1.*fov.fovWidth/fov.canvas_y_grid_size]) 
			# field center RA/Dec
			...
			offsetMagnitude = np.sqrt((boxXSize.value/2.)**2 + (boxYSize.value/2.)**2)*u.deg
			tanAngle = np.arctan2(boxXSize.value/2.,boxYSize.value/2.)*180./np.pi

			box_ul = boxCenter.directional_offset_by((90.-tanAngle)*u.deg,offsetMagnitude)
			box_lr = boxCenter.directional_offset_by((-90.-tanAngle)*u.deg,offsetMagnitude)
			box_ll = boxCenter.directional_offset_by((90.+tanAngle)*u.deg,offsetMagnitude)
			box_ur = boxCenter.directional_offset_by((-90.+tanAngle)*u.deg,offsetMagnitude)
			worlds_box = np.array([[boxCenter.ra.value,boxCenter.dec.value],
...
def update_target_field_locations(request):
	...
	if request.method == 'POST':
		...
		fs_new.fieldSetName += '_custom_{}'.format(timezone.now().strftime("%Y%m%d_%H:%M"))
		fs_new.fieldText = '# FieldName, FieldRA, FieldDec, Telscope, Filter, ExpTime, Priority, Status'

		# retrieve the recent FOV for this user
		fov = CanvasFOV.objects.filter(
			author=request.session.get(
				'rand_session_id','ANON')).latest('created')
		...
		for i in range(len(fieldIDs)):
			...
			tf_new.fieldRA = utility.deg2sex(tf_new.fieldRA_deg/15.)
			...
			newTextRow += '{}, {}, {}, '.format(tf_new.telinstName,tf_new.filterName,tf_new.exposureTime)
	...
	if request.method == 'POST':
		try:
			...
			if decUpLimit > 90:
				...
				decFactor = np.cos(decUpLimit*np.pi/180.)
				raLoLimit = fov.raCenter - fov.fovWidth/2./decFactor
				raUpLimit = fov.raCenter + fov.fovWidth/2./decFactor
			...
			if raLoLimit < 0.:
				raLoLimit = 0.
				raLoLimit_wrap = raLoLimit + 360.
			else:
				raLoLimit_wrap = raLoLimit

			if raUpLimit > 360.:
				raUpLimit = 360.
				raUpLimit_wrap = raUpLimit - 360.
			else:
			...
			fields_queryset = SurveyField.objects.filter(field_id__in=field_names) #.filter(active=True)
			...
			fields_queryset = fields_queryset.filter(Q(ra_cen__gte=raLoLimit) | 
													 Q(ra_cen__gte=raLoLimit_wrap))
			fields_queryset = fields_queryset.filter(Q(ra_cen__lte=raUpLimit) | 
													 Q(ra_cen__lte=raUpLimit_wrap))
...
def init_draw_transients(request):
	if request.method == 'POST':
		try:
			...
			# get all fields
			if 'query_name' in request.POST.keys() and request.POST['query_name'] != 'default':
			...
			transients = transients.filter(Q(ra__gte=raLoLimit) | 
										   Q(ra__gte=raLoLimit_wrap))
			transients = transients.filter(Q(ra__lte=raUpLimit) | 
										   Q(ra__lte=raUpLimit_wrap))
