...
def parse_qcinv_file(filename):
    ...
    qcinv = at.Table(names=('expid','ra','dec','ut','fil','time','secz','psf','sky','cloud','teff','Object'))
...
class DECAM_YSEPZ:
    ...
    def add_args(self, parser=None, usage=None, config=None):
        ...
        # The basics
        parser.add_argument('-f', '--obsfilename', type=str, default=None,
                            help='qinv file from DECam observations')
        parser.add_argument('--yseuser', type=str, default=None,
                            help='YSE username')
        parser.add_argument('--ysepass', type=str, default=None,
                            help='YSE password')
    ...
    def upload_transients(self):
        ...
        if self.options.debug: yseurl = 'http://127.0.0.1:8000'
        ...
        for field in np.unique(data['Object'].values):
            if field not in existingfields:
                ...
                field_add_dict = {
                    ...
                    'width_deg':2.2,
                    'height_deg':2.2}
                r = requests.post(url='%s/api/surveyfields/'%yseurl,json = field_add_dict,
                                 auth=HTTPBasicAuth(self.options.yseuser,self.options.ysepass)).json()
                field_url_dict[field] = r['url']
            else:
                r = requests.get(url='%s/api/surveyfields/?field_id=%s'%(yseurl,field),
                                  auth=HTTPBasicAuth(self.options.yseuser,self.options.ysepass)).json()
        ...
        for i in data.index:
            ...
            if not np.isnan(data['psf'][i]): fwhm = data['psf'][i]
            else: fwhm = None
                
            survey_obs_dict = {'obs_mjd':date_to_mjd(fulltime),
            ...
            r = requests.get(
                url='%s/api/surveyobservations/?survey_field=%s'%(yseurl,data['Object'][i]),
                auth=HTTPBasicAuth(self.options.yseuser,self.options.ysepass)).json()
            duplicate = False
            for d in r['results']:
                if d['photometric_band'] == survey_obs_dict['photometric_band'] and np.abs(d['obs_mjd']-survey_obs_dict['obs_mjd']) < 0.001:
                    duplicate = True
            if duplicate:
