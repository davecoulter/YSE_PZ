...
class YSEMoveFieldsForm(ModelForm):
    ...
    class Meta:
        model = SurveyField
        fields = ["field_id", "targeted_galaxies"]
...
class MoveYSEFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if self.request.is_ajax():
            ...
            try:
                ...
                sc = SkyCoord(
                    form.cleaned_data["ra_str"],
                    form.cleaned_data["dec_str"],
                    unit=(u.hour, u.deg),
                )
            ...
            sfmatch = SurveyField.objects.filter(
                Q(ra_cen__gt=instance.ra_cen - 0.003)
                & Q(ra_cen__lt=instance.ra_cen + 0.003)
                & Q(dec_cen__gt=instance.dec_cen - 0.003)
                & Q(dec_cen__lt=instance.dec_cen + 0.003)
            )
            msb = SurveyFieldMSB.objects.filter(
                name=form.cleaned_data["field_id"].split(".")[0]
            )

            if not len(msb):
                data = {
                    "data": {
                        "warning": "Error : can't find MSB for field %s"
                        % instance.field_id,
                        "email_text": "",
                    },
                    "message": "",
            ...
            if len(sfmatch):
                if sfmatch in msb[0].survey_fields.all():
                    message = (
                        "Survey Field already exists with RA,Dec near (%.7f,%.7f) and is in the MSB for field %s. "
                        % (instance.ra_cen, instance.dec_cen, msb[0].name)
                    )
                else:
                    message = (
                        "Survey Field already exists with RA,Dec near (%.7f,%.7f) and is not yet in the MSB for field %s. "
                        % (instance.ra_cen, instance.dec_cen, msb[0].name)
                    )
    ...
    Thanks very much."""
            ...
            sfcollide = SurveyField.objects.filter(
                Q(
                    ra_cen__gt=instance.ra_cen
                    / np.abs(np.cos(instance.dec_cen * np.pi / 180))
                    - 1.55
                )
                & Q(
                    ra_cen__lt=instance.ra_cen
                    / np.abs(np.cos(instance.dec_cen * np.pi / 180))
                    + 1.55
                )
                & Q(dec_cen__gt=instance.dec_cen - 1.55)
                & Q(dec_cen__lt=instance.dec_cen + 1.55)
            )
            warning = ""
            if len(sfcollide):
                for sfc in sfcollide:
                    msbmatch = SurveyFieldMSB.objects.get(
                        name=sfc.field_id.split(".")[0]
                    )
                    msbsf = msbmatch.survey_fields.filter(field_id=sfc.field_id)
                    if len(msbsf):
                        warning += (
                            "Possible colliding field with ID %s, currently in MSB %s\n"
            ...
            if form.cleaned_data["targeted_transients"]:
                for transient_id in form.cleaned_data["targeted_transients"].split(","):
                    instance.targeted_transients.add(
                        Transient.objects.get(pk=transient_id)
                    )
            ...
            if len(msbsf):
                for msf in msbsf:
                    msb.survey_fields.remove(msf)
            msb.survey_fields.add(instance)
            if len(msb.survey_fields.all()) > 6:
                warning = (
                    warning
                    + "warning : more than 6 survey fields are part of this MSB!"
                )
