...
def yse_pa(msb_name,pa=None):
    ...
    for sf in survey_field_msb.survey_fields.all():
        ...
        transients_in_field_query += [Q(ra__gt = sf.ra_cen-ra_offset.degree) &
    ...
    transients_2best = transients_in_field.filter(Q(status__name='Interesting') & ~Q(tags__in=tg))
    transients_3best = transients_in_field.filter(Q(status__name='FollowupRequested') & ~Q(tags__in=tg))
    transients_4best = transients_in_field.filter(Q(status__name='Following') & ~Q(tags__in=tg))
    ...
    for rotator_angle in rotator_angles:
        ...
        for i,transient_list in enumerate([transients_4best,transients_3best,transients_2best,transients_best]):
            for transient in transient_list:
                cmd = "echo %.7f %.7f | /Users/David/Dropbox/research/YSE_FieldSelection/YSE_scheduler/findcells/pscoords in=sky out=cell ra=%.7f dec=%.7f pa=%.1f dx=0 dy=0 dpa=0 sc=38.856"%(
                ...
                if 'OTA'+ota.decode('utf-8') in goodcells.keys() and cell.decode('utf-8') in goodcells['OTA'+ota.decode('utf-8')]:
                    if i < 3: good_list += [transient.name]
                    else: good_list += [transient.name]*4
    ...
    list_good = np.array(list_full)[best_idx][0] #[np.array(len_good) == np.max(len_good)]
