...
class check_obs:
    ...
    def add_arguments(self, parser=None, usage=None, config=None):
        ...
        parser.add_argument("--ndays", default=6, type=int, help="observation date")
        parser.add_argument(
            "--debug", default=False, action="store_true", help="debug mode"
        )
        parser.add_argument(
            "--dburl",
            default="https://ziggy.ucolick.org/yse/api/",
            type=str,
    ...
    def main(self):
        ...
        offsetcount = 0
        r = requests.get(
            f"{self.options.dburl}surveyobservations/?obs_mjd_gte={nowmjd-self.options.ndays}&obs_mjd_lte={nowmjd}&limit=1000&status_in=Successful&obs_group=YSE",
            auth=HTTPBasicAuth(self.options.dblogin, self.options.dbpassword),
        )
        data = r.json()
        msbs_observed = []
        ...
        if len(active_fields_missing):
            print(
                f"fields {','.join(active_fields_missing)} have not been observed in {self.options.ndays} days"
            )
            self.send_email(active_fields_missing, n_active)
        else:
            print(
                f"all {n_active} fields have been observed in the last {self.options.ndays} days!"
            )
