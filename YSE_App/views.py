...
def dashboard(request):
    ...
    for title, statusname in zip(
        ...
        status = TransientStatus.objects.filter(name=statusname).order_by(
            "-modified_date"
        )
        if len(status) == 1:
            transients = Transient.objects.filter(status=status[0]).order_by(
                "-disc_date"
            )
        else:
...
def personaldashboard(request):
    ...
    for q in queries:
        if q.query:
            if "yse_app_transient" not in q.query.sql.lower():
                continue
            if "name" not in q.query.sql.lower():
                continue
            if not q.query.sql.lower().startswith("select"):
                continue
            ...
            table = TransientTable(
                transientfilter.qs, prefix=q.query.title.replace(" ", "")
            )
            RequestConfig(request, paginate={"per_page": 10}).configure(table)
            tables += [
                (
...
def transient_summary(
    ...
    transient_status_followrequest = TransientStatus.objects.get(
    ...
    transient_followup_form.fields["too_resource"].queryset = (
        ...
        .filter(end_date_valid__gt=timezone.now() - timedelta(days=1))
    ...
    status = TransientStatus.objects.filter(
        name=status_or_query_name.replace("followrequest", "FollowupRequested")
    ...
    if "sort" in request.GET.keys():
        if (
            request.GET["sort"] == "last_obs_date"
            or request.GET["sort"] == "-last_obs_date"
        ):
...
"""
            if request.GET["sort"] == "-last_mag":
                is_descending = True
            else:
                is_descending = False
            transients = transients.annotate(last_mag=RawSQL(raw_query, ())).order_by(
                ("-" if is_descending else "") + "last_mag"
            )
        else:
    ...
    if transient_followup_form.fields["valid_start"].initial:
        context["followup_initial_dates"] = (
            transient_followup_form.fields["valid_start"].initial.strftime(
                "%m/%d/%Y HH:MM"
            ),
            transient_followup_form.fields["valid_stop"].initial.strftime(
                "%m/%d/%Y HH:MM"
            ),
        )
...
def followup(request):
    ...
    table_list = []
    for t in telescopes:
        followup_transients = TransientFollowup.objects.filter(
            Q(too_resource__telescope__name=t)
            | Q(classical_resource__telescope__name=t)
            | Q(queued_resource__telescope__name=t)
        )
        followuptransientfilter = FollowupFilter(
            request.GET, queryset=followup_transients, prefix=t
        )
...
def transient_tags(request):
    ...
    context = {"all_transient_tags": all_transient_tags}
...
def get_transient_tags(request):
    ...
    serializer = TransientSerializer(
        transient_objs, context={"request": request}, many=True
    )
    return_data = {}
...
def calendar(request):
    ...
    context = {"all_dates": all_dates, "user_colors": user_colors}
...
def yse_oncall_calendar(request):
    ...
    users = (
        all_dates.order_by("-on_call_date")
    ...
    for i, u in enumerate(
        users
    ):  # User.objects.all().exclude(username='admin').order_by()):
        # user_colors[u.username] = colors[i % len(colors)]
    ...
    context = {
        ...
        "date_end": (datetime.datetime.now() + datetime.timedelta(1))
        .__str__()
        .split()[0],
    }
...
def observing_calendar(request):
    ...
    context = {"all_dates": all_dates, "telescope_colors": telescope_colors}
...
def too_calendar(request):
    ...
    first_calendar_date = (datetime.datetime.utcnow() - datetime.timedelta(60)).replace(
        tzinfo=pytz.UTC
    )
    last_calendar_date = (datetime.datetime.utcnow() + datetime.timedelta(60)).replace(
        tzinfo=pytz.UTC
    )
    all_too_resources = ToOResource.objects.filter(
        ~Q(end_date_valid__lt=first_calendar_date)
        & ~Q(begin_date_valid__gt=last_calendar_date)
    ).select_related()
    ...
    for i, c in enumerate(all_too_resources.select_related()):
        ...
        if c.begin_date_valid.replace(tzinfo=pytz.UTC) > first_calendar_date:
            date_start = c.begin_date_valid
        else:
            date_start = first_calendar_date

        if c.end_date_valid.replace(tzinfo=pytz.UTC) > last_calendar_date:
            date_end = last_calendar_date
        else:
            date_end = c.end_date_valid
    ...
    context = {"all_resources": all_resources, "telescope_colors": telescope_colors}
...
def too_requests(request, telescope, pi_name):
    ...
    too_resource = (
        ToOResource.objects.filter(telescope__name=telescope.replace("_", " "))
        .filter(begin_date_valid__lte=nowdate)
        .filter(end_date_valid__gte=nowdate)
        .select_related()
    )
    if pi_name != "None":
    ...
    follow_requests = (
        TransientFollowup.objects.filter(too_resource=too_resource[0])
    ...
    followup_table = ToOFollowupTable(
        followuptransientfilter.qs,
        prefix=telescope.replace("_", ""),
    ...
    night_start_12 = tel.twilight_evening_nautical(time, which="previous").isot.split(
        "T"
    )[-1][:-7]
    night_start_18 = tel.twilight_evening_astronomical(
        time, which="previous"
    ).isot.split("T")[-1][:-7]
    night_end_18 = tel.twilight_morning_astronomical(time, which="previous").isot.split(
        "T"
    )[-1][:-7]
    night_end_12 = tel.twilight_morning_nautical(time, which="previous").isot.split(
        "T"
    )[-1][:-7]
...
def yse_home(request):
    ...
    fastrising_transients = fastrising_transient_queryset(ndays=7).filter(
        tags__name="YSE"
    )
    fastrisingtransientfilter = TransientFilter(
        request.GET, queryset=fastrising_transients, prefix="ysefastrise"
    )
    table_fastrising = YSERisingTransientTable(
        fastrisingtransientfilter.qs, prefix="ysefastrise"
    )
    ...
    risingtransientfilter = TransientFilter(
        request.GET, queryset=rising_transients, prefix="yserise"
    )
    ...
    transients = (
        Transient.objects.filter(tags__name="YSE")
        .filter(~Q(status__name="Ignore"))
        .order_by("-disc_date")
    )
    ...
    ztftransients = (
        Transient.objects.filter(tags__name="ZTF in YSE Fields")
        .filter(~Q(status__name="Ignore"))
        .order_by("-disc_date")
    )
    ztftransientfilter = TransientFilter(
        request.GET, queryset=ztftransients, prefix="yseztf"
    )
    ...
    transients_follow = (
        Transient.objects.filter(tags__name="YSE")
        .order_by("-disc_date")
        .filter(Q(status__name="FollowupRequested") | Q(status__name="Following"))
    )
    transientfilter_follow = TransientFilter(
        request.GET, queryset=transients_follow, prefix="yse_follow"
    )
    ...
    obsnights = (
        ObservingResourceService.GetAuthorizedClassicalResource_ByUser(request.user)
        .filter(
            begin_date_valid__lte=datetime.datetime.utcnow() + datetime.timedelta(5)
        )
        .filter(
            begin_date_valid__gte=datetime.datetime.utcnow() - datetime.timedelta(1)
        )
        .select_related()
        .order_by("begin_date_valid")
    )
    ...
    ut_obs_date = (datetime.datetime.utcnow() + datetime.timedelta(1)).strftime(
        "%Y-%m-%d 00:00:00"
    )
    ...
    survey_obs = (
        SurveyObservation.objects.filter(
            Q(mjd_requested__gte=date_to_mjd(sunset_forobs) - 0.1)
            | Q(obs_mjd__gte=date_to_mjd(sunset_forobs) - 0.1)
        )
        .filter(
            Q(mjd_requested__lte=date_to_mjd(sunrise_forobs) + 0.1)
            | Q(obs_mjd__lte=date_to_mjd(sunrise_forobs) + 0.1)
        )
        .filter(survey_field__instrument__name__startswith="GPC")
        .select_related()
    )
    field_pk = survey_obs.values("survey_field").distinct()
    survey_fields_tonight = (
        SurveyField.objects.filter(pk__in=field_pk)
        .filter(~Q(obs_group__name="ZTF"))
        .select_related()
    )
    ...
    nowdate = datetime.datetime.utcnow()
    on_call = YSEOnCallDate.objects.filter(
        on_call_date__gte=nowdate - timedelta(0.5)
    ...
    context = {
        ...
        "transient_fastrising_table": (
            table_fastrising,
            "ysefastrise",
            fastrisingtransientfilter,
        ),
...
def yse_observing_calendar(request):
    ...
    todaydate = dateutil.parser.parse(
    ...
    for i, date in enumerate(date_list):
        ...
        if not len(survey_obs):
            continue
        ztf_obs_ids = (
            survey_obs.filter(obs_mjd__isnull=False)
            .values_list("survey_field__ztf_field_id", flat=True)
            .distinct()
        )
        ztf_sched_ids = (
            survey_obs.filter(obs_mjd__isnull=True)
            .values_list("survey_field__ztf_field_id", flat=True)
            .distinct()
        )
        ...
        for z in ztf_obs_ids:
            filters = (
                survey_obs.filter(survey_field__ztf_field_id=z)
                .values_list("photometric_band__name", flat=True)
                .distinct()
            )
            ztf_obs_str += "%s: %s; " % (
                z.__str__(),
                ",".join([f.__str__() for f in filters]),
            )
        ztf_sched_str = ""
...
def decam_observing_calendar(request):
    ...
    for i, date in enumerate(date_list):
        ...
        decam_obs_ids = (
            survey_obs.filter(obs_mjd__isnull=False)
            .values_list("survey_field__field_id", flat=True)
            .distinct()
        )
        decam_sched_ids = (
            survey_obs.filter(obs_mjd__isnull=True)
        ...
        for z in decam_obs_ids:
            filters = (
                survey_obs.filter(survey_field__field_id=z)
                .filter(survey_field__obs_group__name="DECAT")
                .values_list("photometric_band__name", flat=True)
                .distinct()
            )
        ...
        for z in decam_sched_ids:
            if z in decam_obs_ids:
                continue
            filters = (
                survey_obs.filter(survey_field__decam_field_id=z)
                .filter(survey_field__obs_group__name="DECAT")
                .values_list("photometric_band__name", flat=True)
                .distinct()
            )
...
def observing_night(request, telescope, obs_date, pi_name):
    ...
    # get follow requests for telescope/date
    classical_obs_date = (
        ClassicalObservingDate.objects.filter(obs_date__startswith=obs_date)
        .filter(resource__telescope__name=telescope.replace("_", " "))
        .select_related()
    )
    if pi_name != "None":
        classical_obs_date = classical_obs_date.filter(
            resource__principal_investigator__name=pi_name
        )
    ...
    follow_requests = (
        TransientFollowup.objects.filter(
            classical_resource=classical_obs_date[0].resource
        )
        .filter(valid_start__lte=classical_obs_date[0].resource.begin_date_valid)
        .filter(valid_stop__gte=classical_obs_date[0].resource.end_date_valid)
        .select_related()
    )

    followuptransientfilter = FollowupFilter(
        request.GET, queryset=follow_requests, prefix=telescope
    )

    followup_table = ObsNightFollowupTable(
        followuptransientfilter.qs,
        prefix=telescope,
...
def yse_observing_night(request, obs_date):
    ...
    night_start_12 = tel.twilight_evening_nautical(time, which="next").isot.split("T")[
        -1
    ][:-7]
    night_start_18 = tel.twilight_evening_astronomical(time, which="next").isot.split(
        "T"
    )[-1][:-7]
    night_end_18 = tel.twilight_morning_astronomical(time, which="next").isot.split(
        "T"
    )[-1][:-7]
    night_end_12 = tel.twilight_morning_nautical(time, which="next").isot.split("T")[
        -1
    ][:-7]
    ...
    context["obs_date_str"] = datetime.datetime(
        int(obs_date.split("-")[0]),
        int(obs_date.split("-")[1]),
        int(obs_date.split("-")[2]),
    ).strftime("%m/%d/%Y")
...
def download_target_list(request, telescope, obs_date):
    ...
    # get follow requests for telescope/date
    classical_obs_date = ClassicalObservingDate.objects.filter(
        obs_date__startswith=obs_date
    ).filter(resource__telescope__name=telescope.replace("_", " "))
    follow_requests = (
        TransientFollowup.objects.filter(
            classical_resource=classical_obs_date[0].resource
        ...
        .filter(valid_start__lte=classical_obs_date[0].obs_date)
        .filter(valid_stop__gte=classical_obs_date[0].obs_date)
    )
    ...
    for f in follow_requests:
        comments = ";".join(
            [l.comment for l in Log.objects.filter(transient_followup=f)]
        )
        if f.transient.recent_mag():
            content += "%s  %s %s 2000 mag = %.2f comment = %s\n" % (
                f.transient.name.ljust(20),
                f.transient.CoordString()[0].replace(":", " "),
                f.transient.CoordString()[1].replace(":", " "),
                float(f.transient.recent_mag()),
                comments,
            )
    ...
    response["Content-Disposition"] = (
        "attachment; filename=%s" % "%s_%s.txt" % (telescope, obs_date)
    )
...
def download_targets_and_finders(request, telescope, obs_date):
    ...
    for f in follow_requests:
        ...
        for offdict in offdictlist:
            content_offsets += (
                "%s  %s %s 2000 mag = %.2f raoffset=%.2f decoffset=%.2f\n"
                % (
                    offdict["id"].ljust(20),
                    offdict["ra"],
                    offdict["dec"],
                    float(offdict["mag"]),
                    float(offdict["ra_off"]),
                    float(offdict["dec_off"]),
                )
            )
...
def transient_detail(request, slug):
    ...
    if len(alternate_transient) and not len(transient):
        transient = Transient.objects.filter(
            name=alternate_transient[0].transient.name
        ).select_related()
        # return redirect('/transient_detail/%s/'%transient[0].slug)
        return HttpResponseRedirect(
            reverse_lazy("transient_detail", kwargs={"slug": transient[0].slug})
        )
    ...
    if len(transient) == 1:
            ...
            if att.name == "GW Candidate":
                ...
                if len(gwcand):
                    gwimages = GWCandidateImage.objects.filter(
                        gw_candidate__name=gwcand[0].name
                    )
        ...
        if followups:
            for i in range(len(followups)):
                followups[i].observation_set = TransientObservationTask.objects.filter(
                ...
                comments = Log.objects.filter(
                    transient_followup=followups[i].id
                ).select_related()
                comment_list = []
                for c in comments:
                    comment_list += [c.comment]
                if len(comments):
                    followups[i].comment = "; ".join(comment_list)
                # else:
        #   followups = None
        ...
        if hostdata:
            ...
            ramin, ramax, decmin, decmax = getRADecBox(
                transient_obj.hostdata.ra, transient_obj.hostdata.dec, size=1 / 60.0
            )
        ...
        if hostphotdata:
            transient_obj.hostphotdata = hostphotdata
        ...
        allphotdata = view_utils.get_all_phot_for_transient(
        ...
        has_new_comment = (
            len(
                Log.objects.filter(transient=transient_obj).filter(
                    modified_date__gt=datetime.datetime.now() - datetime.timedelta(1)
                )
            )
            > 0
        )
        ...
        context = {
            ...
            "diff_images": TransientDiffImage.objects.filter(
                phot_data__photometry__transient__name=transient_obj.name
            ),
            "classical_resource_form": classical_resource_form,
            "too_resource_form": too_resource_form,
        ...
        if transient_obj.postage_stamp_file:
            context["qub_candidate"] = transient_obj.postage_stamp_file.split("/")[
                -1
            ].split("_")[0]
        ...
        if (
            not len(tns_submit_logs)
            and "_cand" in transient_obj.name
            and "DECAT" in list(assigned_transient_tags.values_list("name", flat=True))
        ):
            submit_to_tns = True
        else:
            submit_to_tns = False
...
def download_data(request, slug):
    ...
    data[transient[0].name]["transient"] = json.loads(
        serializers.serialize("json", transient, use_natural_foreign_keys=True)
    )

    if transient[0].host:
        host = Host.objects.filter(id=transient[0].host.id)
        data[transient[0].name]["host"] = json.loads(
            serializers.serialize("json", host, use_natural_foreign_keys=True)
        )

    # Get photometry by user & transient
    authorized_phot = PhotometryService.GetAuthorizedTransientPhotometry_ByUser_ByTransient(
        user, transient[0].id
    )
    if len(authorized_phot):
        data[transient[0].name]["photometry"] = json.loads(
            serializers.serialize(
                "json", authorized_phot, use_natural_foreign_keys=True
            )
        )

        # Get data points
        for p, pd in zip(
            authorized_phot, range(len(data[transient[0].name]["photometry"]))
        ):
            photdata = PhotometryService.GetAuthorizedTransientPhotData_ByPhotometry(
                user, p.id, includeBadData=True
            )
            data[transient[0].name]["photometry"][pd]["data"] = json.loads(
                serializers.serialize("json", photdata, use_natural_foreign_keys=True)
            )

    # Get spectra by user & transient
    authorized_spectra = SpectraService.GetAuthorizedTransientSpectrum_ByUser_ByTransient(
        user, transient[0].id, includeBadData=True
    )
    if len(authorized_spectra):
        data[transient[0].name]["spectra"] = json.loads(
            serializers.serialize(
                "json", authorized_spectra, use_natural_foreign_keys=True
            )
        )

        for s, sd in zip(
            authorized_spectra, range(len(data[transient[0].name]["spectra"]))
        ):
            specdata = SpectraService.GetAuthorizedTransientSpecData_BySpectrum(
                user, s.id, includeBadData=True
            )
            if specdata:
                data[transient[0].name]["spectra"][sd]["data"] = json.loads(
                    serializers.serialize(
                        "json", specdata, use_natural_foreign_keys=True
                    )
                )
...
def download_photometry(request, slug):
    ...
    for k in data[transient[0].name]["transient"][0]["fields"].keys():
        if k not in ["created_by", "modified_by", "candidate_hosts"]:
            content += "# %s: %s\n" % (
                k.upper(),
                data[transient[0].name]["transient"][0]["fields"][k],
            )
    ...
    if len(authorized_phot):
        ...
        for p, pd in zip(
            ...
            telescope = data[transient[0].name]["photometry"][pd]["fields"][
                "instrument"
            ].split(" - ")[0]
            instrument = data[transient[0].name]["photometry"][pd]["fields"][
                "instrument"
            ].split(" - ")[1]
            telescope = telescope.replace(" ", "_")
            ...
            for d in data[transient[0].name]["photometry"][pd]["data"]:
                ...
                if (
                    d["fields"]["flux"]
                    and d["fields"]["flux_zero_point"]
                    and not d["fields"]["mag"]
                ):
                    if d["fields"]["flux_err"]:
                        flux_err = d["fields"]["flux_err"]
                    else:
                        flux_err = 0

                    flux = d["fields"]["flux"] * 10 ** (
                        0.4 * (d["fields"]["flux_zero_point"] - 27.5)
                    )
    ...
    response = HttpResponse(content, content_type="text/plain")
    response["Content-Disposition"] = (
        "attachment; filename=%s" % "%s_data.snana.txt" % slug
    )
...
def download_bulk_photometry(request, query_title):
    ...
    if len(query):
        ...
        transients = Transient.objects.filter(name__in=(x[0] for x in cursor)).order_by(
            "-disc_date"
        )
        cursor.close()
    ...
    for transient in transients:
        ...
        data[transient.name]["transient"] = json.loads(
            serializers.serialize(
                "json",
                Transient.objects.filter(name=transient.name),
        ...
        for k in data[transient.name]["transient"][0]["fields"].keys():
            if k not in ["created_by", "modified_by", "candidate_hosts"]:
                content += "# %s: %s\n" % (
                    k.upper(),
                    data[transient.name]["transient"][0]["fields"][k],
                )
        ...
        if len(authorized_phot):
            data[transient.name]["photometry"] = json.loads(
                serializers.serialize(
                    "json", authorized_phot, use_natural_foreign_keys=True
                )
            )
            ...
            # Get data points
            for p, pd in zip(
                authorized_phot, range(len(data[transient.name]["photometry"]))
            ):

                telescope = data[transient.name]["photometry"][pd]["fields"][
                    "instrument"
                ].split(" - ")[0]
                instrument = data[transient.name]["photometry"][pd]["fields"][
                    "instrument"
                ].split(" - ")[1]
                ...
                data[transient.name]["photometry"][pd]["data"] = json.loads(
                    serializers.serialize(
                        "json", photdata, use_natural_foreign_keys=True
                    )
                )
    ...
    response[
        "Content-Disposition"
    ] = "attachment; filename=YSEPZ_transient_photometry.zip"
    return response
...
def download_spectra(request, slug):
            ...
            if specdata:
                ...
                archive.writestr(
                    ...
                    % (
                        transient.name,
                        s.instrument.name,
                        s.obs_date.isoformat().split("T")[0],
                    ),
                    content,
                )
    ...
    response["Content-Disposition"] = (
        "attachment; filename=%s_spectra.zip" % transient.name
    )
    return response
...
def upload_spectrum(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            tspec = (
                TransientSpectrum.objects.filter(transient=transient)
            ...
            specdict = {
                ...
                "ra": form.data["ra"],
                "dec": form.data["dec"],
                "obs_date": form.data["obs_date"],
                "obs_group": ObservationGroup.objects.filter(id=form.data["obs_group"])[
                    0
                ],
            ...
            if form.data["data_quality"]:
                specdict["data_quality"] = DataQuality.objects.get(
                    pk=form.data["data_quality"]
                )
            ...
            return redirect(
                "transient_detail", slug=transient.slug
            )  # HttpResponseRedirect(reverse_lazy('transient_detail',transient.slug))
    else:
        form = SpectrumUploadForm()
    return render(
        request, "YSE_App/form_snippets/spectrum_upload_form.html", {"form": form}
    )
...
class SearchResultsView(ListView):
    ...
    def get_context_data(self):
        ...
        transientfilter = TransientFilter(
            self.request.GET, queryset=transients, prefix=""
        )
        table = TransientTable(transientfilter.qs, prefix="")
        RequestConfig(self.request, paginate={"per_page": 10}).configure(table)
        context["transient_search_results"] = (
            table,
            "Search Results",
            "Search Results",
            transientfilter,
...
def ztf_forced_phot(request, slug):
    ...
    old_log = Log.objects.filter(transient=transient).filter(
        modified_date__gt=datetime.datetime.utcnow().replace(tzinfo=pytz.UTC)
        - datetime.timedelta(0.5)
    )
    if len(old_log):
        context = {
            "msg": "error: forced phot was requested %.1f hours ago (must be >12)"
            % (
                (
                    datetime.datetime.utcnow().replace(tzinfo=pytz.UTC)
                    - old_log[0].modified_date
                ).seconds
                / 3600.0
            )
        }
    ...
    ztf = ZTF_Forced_Phot.ZTF_Forced_Phot(
        ztf_email_address="%s@gmail.com" % djangoSettings.SMTP_LOGIN,
        ztf_email_password=djangoSettings.SMTP_PASSWORD,
        ztf_email_imapserver="imap.gmail.com",
        ztf_user_address="%s@gmail.com" % djangoSettings.SMTP_LOGIN,
    ...
    log_file_name = ztf.run_ztf_fp(
        ...
        days=60,
        decl=transient.dec,
        directory_path=djangoSettings.ZTFTMPDIR,
        do_plot=False,
        emailcheck=0,
        fivemindelay=60,
        jdend=None,
