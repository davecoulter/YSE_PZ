...
class upload:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        parser.add_option(
            "-i", "--inputfile", default=None, type="string", help="input file "
        )
        parser.add_option("--trid", default=None, type="string", help="transient ID")
        parser.add_option(
            "--status",
            default="Following",
            type="string",
        ...
        parser.add_option(
            "--instrument", default="GPC1", type="string", help="instrument name"
        )
        parser.add_option(
            "--forcedphot", default=0, type="int", help="set to 1 if forced photometry"
        )
        parser.add_option(
            "--diffim", default=0, type="int", help="set to 1 if difference imaged"
        )
        parser.add_option(
            "--fluxzpt", default=None, type="float", help="flux zero point"
        )
        parser.add_option(
            "-u",
            "--useheader",
...
less than this, in the same filter/instrument are treated as the same data.	 Allows updates to the photometry""",
    ...
    def uploadHeader(self, sn, db=None):
        ...
        if not obsid or not statusid:
            raise RuntimeError("Error : obsgroup or status not found in DB!")

        transgetcmd = "http -a %s:%s GET %s/transients/" % (
            self.options.login,
            self.options.password,
            self.options.postURL,
        )
        ...
        if not transid:
            if type(sn.RA) == float:
                ...
                basicdatacmd = (
                    "http -a %s:%s POST %s/transients/ name='%s' ra='%s' dec='%s' status='%s' obs_group=%s"
                    % (
                        self.options.login,
                        self.options.password,
                        self.options.postURL,
    ...
    def uploadSNANAPhotometry(self, db=None):
        ...
        if type(sn.RA) == float:
            ...
            transientdict, photdict = self.parsePhotHeaderData(
                transname, sn.RA.split()[0], sn.DECL.split()[0]
            )
            # get the filter IDs
        ...
        for mjd, flux, fluxerr, mag, magerr, flt, i in zip(
            ...
            if (
                "SEARCH_PEAKMJD" in sn.__dict__.keys()
    ...
    def uploadBasicSpectrum(self, db=None):
        ...
        for key in requiredkeyslist:
            if key not in SpecHeader.keys():
                raise RuntimeError(
                    """Error: required key %s not in spectrum header.
				Format should be
				# key keyval"""
                    % key
                )
...
class DBOps:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        parser.add_option(
            "--diffim", default=0, type="int", help="set to 1 if forced photometry"
        )
        parser.add_option(
            "--obsgroup",
            default="Foundation",
