...
def main(transients=None):
    if transients is None:
        transients = Transient.objects.filter(tags__name="YSE").filter(
            name__startswith="20"
        )  # .filter(Q(host__isnull=True) | Q(host__redshift__isnull=True))
    for t in transients:
        print(t)
        sc = [SkyCoord(t.ra, t.dec, unit=u.deg)]
        ghost_hosts = getTransientHosts(
            [t.name], sc, verbose=True, starcut="gentle", ascentMatch=False
        )
