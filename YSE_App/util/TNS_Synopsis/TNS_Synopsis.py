                        ...
                        for table in tables:
                            ...
                            for datarow in data:
                                ...
                                if photkeydict["inst"] in header:
                                    tinst = np.append(
                                        tinst, datarow[header == photkeydict["inst"]]
                                    )
                    ...
                    if post:
                        ...
                        if not dbid:
                            ...
                            for ins in np.unique(tinst):
                                ...
                                if not instrumentid:
                                    instrumentid = db.get_ID_from_DB(
                                        "instruments", "Unknown"
                                    )
                                if not instrumentid:
                                ...
                                phottableid = db.post_object_to_DB(
                                    "photometry", phottabledict
                                )

                                for f in np.unique(tfilt):
                                    bandid = db.get_ID_from_DB("photometricbands", f)
                                    if not bandid:
                                        bandid = db.post_object_to_DB(
                                            "band",
                                            {"name": f, "instrument": instrumentid},
                                        )

                                    # put in the photometry
                                    try:
                                        for m, me, f, fe, od, df in zip(
                                            ...
                                            if not m and not me and not f and not fe:
                                                continue
                                            ...
                                            if m:
                                                photdatadict["mag"] = m
                                            if me:
                                                photdatadict["mag_err"] = me
                                            if f:
                                                photdatadict["flux"] = f
                                            if fe:
                                                photdatadict["flux_err"] = fe
                                            if df:
                                                photdatadict["discovery_point"] = 1
                                            photdataid = db.post_object_to_DB(
                                                "photdata", photdatadict
                                            )
                                    except:
