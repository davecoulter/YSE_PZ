...
class lcogt(object):
    def __init__(self, username, password, progid, telescop, start_date, end_date):
        ...
        self.params = {
            ...
            "constraints": {"max_airmass": 2.5, "min_lunar_distance": 15},
            "strategy": {
                "default": {
                    "type": "default",
                    "proposal": [{"name": progid, "obstype": "NORMAL"}],
                    "filters": ["up", "gp", "rp", "ip"],
        ...
        if "soar" in telescop:
            ...
            self.params["constraints"] = {"max_airmass": 1.7, "min_lunar_distance": 30}
        ...
        self.constants = {
            "zpt": {"up": 20.5665, "gp": 23.2249, "rp": 23.1314, "ip": 22.8465}
        }
    ...
    def get_spectral_calibrations(self, date, telid, site, outrootdir="", funpack=True):
        ...
        response = requests.get(
            self.params["uri"]["archive"]["frames"], params=params, headers=headers
        )
    ...
    def get_requestgroups(
        self, propid=None, sdate=None, edate=None, obstype=None, itype=None
    ):
        ...
        for pid in propids:
            ...
            # Now do request
            response = requests.get(
                self.params["uri"]["request"]["request"], params=params, headers=headers
            ).json()
    ...
    def download_obslist(
        ...
        for frame in obslist:
            ...
            if use_basename:
                ...
                filename = target + "." + date + "." + filt + "." + idnum + ".fits.fz"
            fullfilename = outrootdir + "/" + filename
            if not os.path.exists(outrootdir):
                shutil.os.makedirs(outrootdir)
            if (
                not os.path.exists(fullfilename)
                and not os.path.exists(fullfilename.strip(".fz"))
                and funpack
    ...
    def make_instrument_configs(self, filt, exptime, strat):
        ...
        # This creates a spectroscopy observing element for FLOYDS
            ...
            configuration = {
                ...
                "rotator_mode": "VFLOAT",
                "extra_params": {},
                "optical_elements": {"slit": strat["slit"]},
            }
            ...
            configuration = {
                ...
                "optical_elements": {"slit": strat["slit"], "grating": "SYZY_400"},
            }
    ...
    def make_window(self, start, duration):
        ...
        window = [
            {"start": self.start_date.strftime(fmt), "end": self.end_date.strftime(fmt)}
        ]
        return window
    ...
    def make_requests(self, obj, ra, dec, exptime, strat):
        ...
        requests = [
            {"location": location, "windows": window, "configurations": configurations}
        ]
    ...
    def make_obs_request(self, obj, ra, dec, exptime, strategy="default", propidx=""):
...
def main(name, ra, dec, exptime, telescop, start_date, end_date):
    ...
    lco = lcogt(
        djangoSettings.LCOGTUSER,
        djangoSettings.LCOGTPASS,
        progid,
        telescop,
        start_date,
        end_date,
    ...
    response = lco.make_obs_request(
        name, ra, dec, exptime, strategy="spectroscopy", propidx=progid
    )
