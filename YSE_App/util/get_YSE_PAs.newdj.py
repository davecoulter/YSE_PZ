...
class YSE_PA:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        parser.add_argument(
            "--yseurl",
            type=str,
            default="https://ziggy.ucolick.org/yse",
            help="YSE URL",
        )
        parser.add_argument(
            "-u", "--yseuser", type=str, default="psobs", help="YSE user name"
        )
        parser.add_argument(
            "-p", "--ysepass", type=str, default="!ps-yse!", help="YSE password"
        )
        parser.add_argument(
            "-c", "--cron", action="store_true", help="use if calling from cron (e.g.)"
        )
        parser.add_argument(
            "--goodcellsfile",
            type=str,
            default="/home/schultz/YSE/getPA/good_cells.txt",
            help="file with the good PS1 cells",
        )
        parser.add_argument(
            "--pscoords",
            type=str,
            default="/home/schultz/YSE/getPA/pscoords",
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="optional comma-separated list giving fields that need PAs",
        )
        parser.add_argument(
            "--mjds_to_schedule",
            type=str,
            default=None,
            help="optional comma-separated list giving the times that each field should be scheduled.  Length must match length of --fieldlist argument",
        )
    ...
    def yse_pa(
        ...
        for f, ra, ddec in zip(msb_fields, msb_ras, msb_decs):
            ...
            r = requests.get(
                f"{self.options.yseurl}/box_search/{ra}/{ddec}/1.65/",
        ...
        if (
            self.options.mjds_to_schedule is None
            or msb_fields[0].split(".")[0] not in self.options.fieldlist
        ):
            ...
            time = [
                mjd_to_date(
                    self.options.mjds_to_schedule[
                        self.options.fieldlist == msb_fields[0].split(".")[0]
                    ][0]
                )
            ]
        ...
        for rotator_angle in rotator_angles:
            ...
            for i, transient_list, transient_ra, transient_dec in zip(
                ...
                [
                    transients_4best_dec,
                    transients_3best_dec,
                    transients_2best_dec,
                    transients_best_dec,
                ],
            ):
                for transient, tra, tdec in zip(
                    transient_list, transient_ra, transient_dec
                ):
                    cmd = (
                        "echo %.7f %.7f | %s in=sky out=cell ra=%.7f dec=%.7f pa=%.1f dx=0 dy=0 dpa=0 sc=38.856"
                        % (
                            tra,
                            tdec,
                            self.options.pscoords,
                            msb_ras[0],
                            msb_decs[0],
                            pa,
                        )
                    ...
                    if (
                        "OTA" + ota.decode("utf-8") in self.goodcells.keys()
                        and cell.decode("utf-8")
                        in self.goodcells["OTA" + ota.decode("utf-8")]
                    ):
                        if i < 3:
                            good_list += [transient]
                        else:
                            good_list += [transient] * 4
    ...
    def yse_pa_new(
        ...
        for pa in palist:
            ...
            for i, transient_list, transient_ra, transient_dec, transient_field in zip(
                ...
                [
                    transients_4best_field,
                    transients_3best_field,
                    transients_2best_field,
                    transients_best_field,
                ],
            ):
                for transient, tra, tdec, tfield in zip(
                    transient_list, transient_ra, transient_dec, transient_field
                ):

                    msbr, msbd = (
                        msb_ras[msb_fields == tfield][0],
                        msb_decs[msb_fields == tfield][0],
                    )
                    cmd = (
                        "echo %.7f %.7f | %s in=sky out=cell ra=%.7f dec=%.7f pa=%.1f dx=0 dy=0 dpa=0 sc=38.856"
    ...
    def get_requested_fields(self):
        ...
        r = requests.get(
            f"{self.options.yseurl}/api/surveyobservations/?mjd_requested_gte={nowmjd}&mjd_requested_lte={nowmjd+1}&limit=1000",
            auth=HTTPBasicAuth(self.options.yseuser, self.options.ysepass),
        )
        ...
        r = requests.get(
            f"{self.options.yseurl}/api/surveyfields/?limit=3000",
            auth=HTTPBasicAuth(self.options.yseuser, self.options.ysepass),
        )
        ...
        r = requests.get(
            f"{self.options.yseurl}/api/photometricbands/",
            auth=HTTPBasicAuth(self.options.yseuser, self.options.ysepass),
        )
        ...
        field_msbs, fields, obs_mjds, ras, decs, obs_dates = (
            np.array([]),
            np.array([]),
            np.array([]),
            np.array([]),
    ...
    def main(self):
        ...
        r = requests.get(
            f"{self.options.yseurl}/query_api/all_yse_transients/",
            auth=HTTPBasicAuth(self.options.yseuser, self.options.ysepass),
        )
        data_all = json.loads(r.text)["transients"]
        r = requests.get(
            f"{self.options.yseurl}/query_api/yse_transients_goodcell/",
            auth=HTTPBasicAuth(self.options.yseuser, self.options.ysepass),
        )
        ...
        for f in fielddict.keys():
            best_pa, list_good = self.yse_pa_new(
                fielddict[f][0],
                fielddict[f][1],
                fielddict[f][2],
                transients_all,
        ...
        for k, best_pa in zip(fielddict.keys(), best_pa_list):
            ...
            # if best_pa is not None:
            #    print(f"{k} {best_pa:03} {'.'.join(fieldfilts[k])}")
            # else:
            #    print(f"{k} None  {'.'.join(fieldfilts[k])}")
...
if __name__ == "__main__":
    ...
    if ys.options.mjds_to_schedule is not None:
        ys.options.mjds_to_schedule = np.array(
            ys.options.mjds_to_schedule.split(",")
        ).astype(float)
        if ys.options.fieldlist is None:
            raise RuntimeError(
                "if --mjds_to_schedule is given, fieldlist must be as well"
            )
        if len(ys.options.fieldlist) != len(ys.options.mjds_to_schedule):
            raise RuntimeError(
                "--fieldlist and --mjds_to_schedule must be the same length"
            )
