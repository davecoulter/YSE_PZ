...
def main(doSpec=False):
    ...
    if doSpec:
        ...
        tq = Transient.objects.annotate(
            num_spec=Count("transientspectrum__obs_group")
        ).filter(
            num_spec__gt=1
        )  # .\
        ...
        for t in tq:
            TNSGetSingle = [("objname", t.name), ("photometry", "0"), ("spectra", "1")]
                ...
                for spec in TransientSpectrum.objects.filter(transient__name=t.name):
                    ...
                    for tnsspec in json_data["data"]["reply"]["spectra"]:
                        if (
                            np.abs(
                                (
                                    dateutil.parser.parse(tnsspec["obsdate"]).replace(
                                        tzinfo=pytz.UTC
                                    )
                                    - spec.obs_date
                                ).days
    ...
    Transient.objects.filter(status__name="FollowupFinished").update(
        status=status_ignore
    )
    Transient.objects.filter(status__name="Following").update(status=status_watch)
    Transient.objects.filter(status__name="FollowupRequested").update(
        status=status_watch
    )
    ...
    tp = (
        TransientPhotometry.objects.filter(instrument__name="ACAM1")
        .annotate(num_phot=Count("transientphotdata__mag"))
        .filter(num_phot__gt=2)
    )
    tp.delete()

    # non-public photometry
    non_public_phot = TransientPhotometry.objects.filter(
        ~Q(groups__name="Public") & ~Q(groups__name__isnull=True)
    )
    non_public_phot.delete()
    phot_ids = non_public_phot.values("id")
    non_public_photdata = TransientPhotData.objects.filter(
        Q(photometry__id__in=phot_ids)
    )
    non_public_photdata.delete()

    # non-public spectra
    non_public_spec = TransientSpectrum.objects.filter(
        ~Q(groups__name="Public") & ~Q(groups__name__isnull=True)
    )
    ...
    # tags
    optional_tags = TransientTag.objects.filter(
        ~Q(name="TESS")
        & ~Q(name="YSE")
        & ~Q(name="ZTF in YSE Fields")
    ...
    tp = (
        TransientPhotometry.objects.filter(instrument__name="GPC1")
        .annotate(num_phot=Count("transientphotdata__mag"))
        .filter(num_phot__gt=2)
    )
    tp.delete()
    ...
    for t in Transient.objects.filter(
        ~Q(name__startswith="201") & ~Q(name__startswith="202")
    ):
        try:
            h = t.host
            if h:
