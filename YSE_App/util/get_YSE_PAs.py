...
class YSE_PA:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        parser.add_argument(
            "-u", "--yseuser", type=str, default=None, help="YSE user name"
        )
        parser.add_argument(
            "-p", "--ysepass", type=str, default=None, help="YSE password"
        )
        ...
        for rotator_angle in rotator_angles:
            ...
            for i, transient_list, transient_ra, transient_dec in zip(
                ...
                for transient, tra, tdec in zip(
                    ...
                    cmd = (
                        "echo %.7f %.7f | ./pscoords in=sky out=cell ra=%.7f dec=%.7f pa=%.1f dx=0 dy=0 dpa=0 sc=38.856"
                    ...
                    if (
                        ...
                        in goodcells["OTA" + ota.decode("utf-8")]
                    ):
                        if i < 3:
                            good_list += [transient]
                        else:
    ...
    def get_requested_fields(self):
        ...
        r = requests.get(
            f"{self.options.yseurl}/api/surveyobservations/?mjd_requested_gte={nowmjd-1}&mjd_requested_lte={nowmjd}&limit=1000",
            auth=HTTPBasicAuth(self.options.yseuser, self.options.ysepass),
        )
    ...
    def main(self):
        ...
        for f in fielddict.keys():
            best_pa, list_good = self.yse_pa(
                fielddict[f][0], fielddict[f][1], fielddict[f][2]
            )
