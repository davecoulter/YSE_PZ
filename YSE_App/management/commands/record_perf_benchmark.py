...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        run = plots.append_run(label=label, commit=commit, branch=branch, pages=pages)
        self.stdout.write(
            self.style.SUCCESS(
                f"Appended iteration {run['iteration_index']} ({label}) to metrics_history.json"
            )
        )

        if not options["skip_plots"]:
            wf = plots.write_waterfall_plots(pages)
