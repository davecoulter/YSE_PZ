...
def add_yse_survey_fields(request):
    ...
    for surveylistkey in survey_data.keys():
            ...
            if not isinstance(SurveyField._meta.get_field(surveykey), ForeignKey):
                ...
                if surveykey == "photometric_band":
                    fk = fkmodel.objects.filter(
                        name=survey[surveykey].split("-")[1]
                    ).filter(instrument__name=survey[surveykey].split("-")[0])
                elif surveykey == "survey_field":
                    fk = fkmodel.objects.filter(field_id=survey[surveykey])
                else:
                    fk = fkmodel.objects.filter(name=survey[surveykey])
                if not len(fk):
                    ...
                    sendemail(
                        ...
                        djangoSettings.SMTP_LOGIN,
                        djangoSettings.SMTP_PASSWORD,
                        smtpserver,
                    )
                    continue
        ...
        surveymsb = SurveyFieldMSB.objects.filter(
            name=surveydict["field_id"].split(".")[0]
        )
        if not len(surveymsb):
            ...
            if surveymsb.survey_fields.count() < 6 and not len(
                surveymsb.survey_fields.filter(field_id=dbsurveyfield.field_id)
            ):
                surveymsb.survey_fields.add(dbsurveyfield)
                surveymsb.save()
...
def add_yse_survey_obs(request):
    ...
    for surveylistkey in survey_data.keys():
        ...
        for surveykey in surveykeys:
            ...
            if not isinstance(SurveyObservation._meta.get_field(surveykey), ForeignKey):
                ...
                if surveykey == "photometric_band":
                    ...
                    fk = fkmodel.objects.filter(
                        Q(ra_cen__gt=float(survey["ra_cen"]) - 0.01)
                        & Q(ra_cen__lt=float(survey["ra_cen"]) + 0.01)
                        & Q(dec_cen__gt=float(survey["dec_cen"]) - 0.01)
                        & Q(dec_cen__lt=float(survey["dec_cen"]) + 0.01)
                    )
                    ...
                    if not len(fk):
                        ...
                        msblist = SurveyFieldMSB.objects.filter(
                            survey_fields__in=[fk[0]]
                        )
                        for msb in msblist:
                    ...
                    if not len(fk):
                        try:
                            ztf_id = survey[surveykey].split(".")[0]
                        except:
                            ztf_id = None
                        fk = fkmodel.objects.create(
                            field_id=survey[surveykey],
                            ra_cen=survey["ra_cen"],
                            dec_cen=survey["dec_cen"],
                            instrument=Instrument.objects.get(name="GPC1"),
                            created_by_id=user.id,
                            modified_by_id=user.id,
                            active=1,
                            width_deg=3.1,
        ...
        dbsurveyfield = (
            SurveyObservation.objects.filter(survey_field=surveydict["survey_field"])
            .filter(obs_mjd__gt=float(surveydict["obs_mjd"]) - 0.01)
            .filter(obs_mjd__lt=float(surveydict["obs_mjd"]) + 0.01)
            .filter(photometric_band=surveydict["photometric_band"])
        )
        ...
        if not len(dbsurveyfield):
            ...
            dbsurveyfield = (
                SurveyObservation.objects.filter(
                    survey_field=surveydict["survey_field"]
                )
                .filter(mjd_requested__gt=sunset)
                .filter(mjd_requested__lt=sunrise)
                .filter(photometric_band=surveydict["photometric_band"])
            )
...
def add_transient(request):
    ...
    for transientlistkey in transient_data.keys():
        if transientlistkey == "noupdatestatus":
            continue
        if transientlistkey == "TNS":
            continue
        ...
        if "name" not in transientkeys:
            return_dict = {
                "message": "Error : Transient name not provided for transient %s!"
                % transientlistkey
            }
            return JsonResponse(return_dict)
        ...
        try:
            ...
            for transientkey in transientkeys:
                if (
                    transientkey == "transientphotometry"
                    or transientkey == "transientspectra"
                    or transientkey == "host"
                    or transientkey == "tags"
                    or transientkey == "gw"
                    or transientkey == "non_detect_instrument"
                    or transientkey == "internal_names"
                ):
                    continue

                if not isinstance(Transient._meta.get_field(transientkey), ForeignKey):
                    if transient[transientkey] is not None:
                        transientdict[transientkey] = transient[transientkey]
                else:
                    fkmodel = Transient._meta.get_field(transientkey).remote_field.model
                    if (
                        transientkey == "non_detect_band"
                        and "non_detect_instrument" in transient.keys()
                    ):
                        fk = fkmodel.objects.filter(
                            name=transient[transientkey]
                        ).filter(instrument__name=transient["non_detect_instrument"])
                        if not len(fk):
            ...
            if not len(dbtransient):
                ...
                ramin, ramax, decmin, decmax = getRADecBox(
                    transient["ra"], transient["dec"], size=0.00042
                )
                if "disc_date" in transient.keys():
                    dbtransient = Transient.objects.filter(
                        ...
                        & Q(dec__lt=decmax)
                        & Q(
                            disc_date__gte=dateutil.parser.parse(transient["disc_date"])
                            - datetime.timedelta(365)
                        )
                        & Q(
                            disc_date__lte=dateutil.parser.parse(transient["disc_date"])
                            + datetime.timedelta(365)
                        )
                    )
                else:
                ...
                if len(dbtransient):
                    ...
                    obs_group = ObservationGroup.objects.get(
                        name=transient["obs_group"]
                    )
                    if "TNS" in transient_data.keys() and transient_data["TNS"]:
                        alt_transients = AlternateTransientNames.objects.filter(
                            name=transient["name"]
                        )
                    ...
                    if (
                        "noupdatestatus" in transient_data.keys()
                        and not transient_data["noupdatestatus"]
                    ):
                        if dbtransient[0].status.name == "Ignore":
                            dbtransient[0].status = TransientStatus.objects.filter(
                                name="New"
                            )[0]
                        else:
                            transientdict["status"] = dbtransient[0].status
                    else:
                        transientdict["status"] = dbtransient[0].status
                    if dbtransient[0].postage_stamp_file:
                        transientdict["postage_stamp_file"] = dbtransient[
                            0
                        ].postage_stamp_file
                        transientdict["postage_stamp_ref"] = dbtransient[
                            0
                        ].postage_stamp_ref
                        transientdict["postage_stamp_diff"] = dbtransient[
                            0
                        ].postage_stamp_diff
                        transientdict["postage_stamp_file_fits"] = dbtransient[
                            0
                        ].postage_stamp_file_fits
                        transientdict["postage_stamp_ref_fits"] = dbtransient[
                            0
                        ].postage_stamp_ref_fits
                        transientdict["postage_stamp_diff_fits"] = dbtransient[
                            0
                ...
                if "internal_names" in transient.keys():
                    for internal_name in transient["internal_names"].split(","):
                        ...
                        altgroup = ObservationGroup.objects.filter(
                            name__startswith=internal_name.replace(" ", "")[:3]
                        )
                        if len(altgroup) != 1:
            ...
            sendemail(
                ...
                user.email,
                subject,
                html_msg % (transient["name"], e, exc_tb.tb_lineno),
                djangoSettings.SMTP_LOGIN,
                djangoSettings.SMTP_PASSWORD,
                smtpserver,
            )
    ...
    for transientlistkey in transient_data.keys():
        ...
        dbtransient = Transient.objects.filter(name=transient["name"])
        if not len(dbtransient):
            dbtransient = AlternateTransientNames.objects.filter(
                name=transient["name"]
            )[0].transient
        else:
            dbtransient = dbtransient[0]
        ...
        try:
            ...
            for transientkey in transientkeys:
                if (
                    ...
                    or transientkey == "host"
                    or transientkey == "tags"
                    or transientkey == "gwcandidate"
                    or transientkey == "gwcandidateimage"
                    or transientkey == "non_detect_instrument"
                ):
                    continue
            ...
            if not len(dbtransient):
                # ra/dec box query
                sc = SkyCoord(
                    transient["ra"], transient["dec"], frame="fk5", unit=u.deg
                )
                ramin, ramax, decmin, decmax = getRADecBox(
                    sc.ra.deg, sc.dec.deg, size=5 / 3600.0
                )
                dbtransient = Transient.objects.filter(
                    Q(ra__gte=ramin)
                    & Q(ra__lte=ramax)
            ...
            if "transientphotometry" in transientkeys:
                # do photometry
                add_transient_phot_util(
                    transient["transientphotometry"], dbtransient, user
                )

            if "transientspectra" in transientkeys:
                # spectrum
                add_transient_spec_util(
                    transient["transientspectra"], dbtransient, user
                )
...
def add_gw_candidate_util(gwdict, transient, user):
    ...
    for gwkey in gwdict.keys():
        ...
        if not isinstance(GWCandidate._meta.get_field(gwkey), ForeignKey):
            ...
            if not len(fk):
                # ready to send error emails
                smtpserver = "%s:%s" % (
                    djangoSettings.SMTP_HOST,
                    djangoSettings.SMTP_PORT,
                )
                from_addr = "%s@gmail.com" % djangoSettings.SMTP_LOGIN
                subject = "TNS Transient Upload Failure"
                html_msg = "Alert : YSE_PZ Failed to upload transient %s "
    ...
    for gwtopkey in gwdict["gwcandidateimage"].keys():
        ...
        for gwkey in gwdict["gwcandidateimage"][gwtopkey].keys():
            if not isinstance(GWCandidateImage._meta.get_field(gwkey), ForeignKey):
                ...
                if gwkey == "image_filter":
                    fk = fkmodel.objects.filter(
                        name=gwdict["gwcandidateimage"][gwtopkey][gwkey].split(" - ")[1]
                    ).filter(
                        instrument__name=gwdict["gwcandidateimage"][gwtopkey][
                            gwkey
                        ].split(" - ")[0]
                    )
                    if not len(fk):
                        fk = fkmodel.objects.filter(
                            name=gwdict["gwcandidateimage"][gwtopkey][gwkey]
                        )
                else:
                    fk = fkmodel.objects.filter(
                        name=gwdict["gwcandidateimage"][gwtopkey][gwkey]
                ...
                if not len(fk):
                    ...
                    sendemail(
                        ...
                        % (
                            gwdict["name"],
                            gwdict["gwcandidateimage"][gwtopkey],
                            gwdict["gwcandidateimage"][gwtopkey][gwkey],
                        ),
                        djangoSettings.SMTP_LOGIN,
                        djangoSettings.SMTP_PASSWORD,
                        smtpserver,
        ...
        dbgwimage = GWCandidateImage.objects.filter(gw_candidate=dbgw).filter(
            image_filename=dbgwimagedict["image_filename"]
        )
        ...
        if not isinstance(Host._meta.get_field(hostkey), ForeignKey):
            ...
            if not len(fk):
                ...
                sendemail(
                    ...
                    subject,
                    html_msg
                    % (transient["name"], transient[transientkey], transientkey),
                    djangoSettings.SMTP_LOGIN,
                    djangoSettings.SMTP_PASSWORD,
                    smtpserver,
                )
...
def add_transient_phot_util(photdict, transient, user, do_photdata=True):
    ...
    for k in photdict.keys():
        ...
        transientphot = (
            TransientPhotometry.objects.filter(transient=transient)
            .filter(instrument=instrument)
            .filter(obs_group=obs_group)
        )
        ...
        if do_photdata:
            for k in photometry["photdata"]:
                ...
                band = PhotometricBand.objects.filter(name=p["band"]).filter(
                    instrument__name=photometry["instrument"]
                )
                if len(band):
                    band = band[0]
                else:
                    band = PhotometricBand.objects.filter(name="Unknown")[0]
                ...
                for e in existingphot:
                    if e.photometry.id == transientphot.id:
                        if e.band == band:
                            try:
                                mjd = Time(
                                    e.obs_date.isoformat().split("+")[0], format="isot"
                                ).mjd
                            except:
                            ...
                            if np.abs(mjd - pmjd) < photdict["mjdmatchmin"]:
                                ...
                                if "diffimg" in p.keys():
                                    existing_diff = TransientDiffImage.objects.filter(
                                        phot_data=e
                                    )
                                    ...
                                    if not len(existing_diff):
                                        TransientDiffImage.objects.create(
                                            **p["diffimg"]
                                        )
                                    else:
...
def add_transient_spec_util(specdict, transient, user):
    ...
    for k in specdict.keys():
        ...
        transientspec = (
            ...
            .filter(instrument=instrument)
            .filter(obs_group=obs_group)
            .filter(obs_date=spectrum["obs_date"])
        )
        ...
        for k in specdata.keys():
            ...
            specdata_single = TransientSpecData(
                spectrum=transientspec,
...
def add_transient_phot(request):
    ...
    if (
        "header" in phot_data.keys()
        and "transient" in phot_data.keys()
        and "photheader" in phot_data.keys()
    ):
        hd = phot_data["header"]
    ...
    for k in phot_data.keys():
        if k == "header" or k == "transient" or k == "photheader":
            continue
        p = phot_data[k]
        pmjd = Time(p["obs_date"], format="isot").mjd
        band = PhotometricBand.objects.filter(name=p["band"]).filter(
            instrument__name=ph["instrument"]
        )
        if len(band):
            band = band[0]
        else:
    ...
    try:
        ...
        if not len(transientspec):
            transientspec = TransientSpectrum.objects.create(
                ...
                transient=transient,
                rlap=hd["rlap"],
                redshift=hd["redshift"],
                redshift_err=hd["redshift_err"],
                redshift_quality=hd["redshift_quality"],
                spectrum_notes=hd["spectrum_notes"],
                spec_phase=hd["spec_phase"],
                obs_date=hd["obs_date"],
                data_quality=dq,
                created_by_id=user.id,
                modified_by_id=user.id,
            )
...
def get_transient(request, slug):
    ...
    if t.exists():
        ...
        serializer = TransientSerializer(
            instance=transient, context={"request": Request(request)}
        )
...
def get_rising_transients_box(request, ra, dec, ra_width, dec_width):
    ...
    ramin, ramax, decmin, decmax = getRADecBox(
        float(ra), float(dec), float(ra_width), float(dec_width)
    )
    qs = (
        qs.filter(ra__gt=ramin)
        .filter(ra__lt=ramax)
        .filter(dec__gt=decmin)
        .filter(dec__lt=decmax)
        ...
        .filter(disc_date__gt=datetime.datetime.utcnow() - datetime.timedelta(10))
    )
...
def get_new_transients_box(request, ra, dec, ra_width, dec_width):
    ...
    qs = (
        Transient.objects.all()
    )  # filter(disc_date__gte=datetime.datetime.utcnow()-datetime.timedelta(5)) #.filter(~Q(TNS_spec_class__isnull=True))
    ramin, ramax, decmin, decmax = getRADecBox(
        float(ra), float(dec), float(ra_width), float(dec_width)
    )
    qs = (
...
def query_api(request, query_name):
    ...
    if len(query):
        ...
        try:
            transients = getattr(
                yse_python_queries, unquote(query_name)
            )()  # .python_query)()
        except:
            return Http404("Invalid Query")
    ...
    for vals in field_vals:
        ...
        for f, v in zip(field_names, vals):
            tdict[f] = v
        serialized_transients.append(tdict)
...
""" % (
    ...
    qs = Transient.objects.filter(
        created_date__gte=datetime.datetime.utcnow() - datetime.timedelta(90)
    ).filter(name__in=(x[0] for x in cursor))
