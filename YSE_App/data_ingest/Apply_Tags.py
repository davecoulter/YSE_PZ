        ...
        try:
            ...
            for t in transients:
                ...
                if tag_K2:
                    ...
                    print(
                        "K2 C16 Val: %s; K2 Val Msg: %s"
                        % (is_k2_C16_validated, C16_msg)
                    )
                    print(
                        "K2 C17 Val: %s; K2 Val Msg: %s"
                        % (is_k2_C17_validated, C17_msg)
                    )
                    print(
                        "K2 C19 Val: %s; K2 Val Msg: %s"
                ...
                if tag_TESS and t.disc_date:
                    TESSFlag = tess_obs(
                        t.ra, t.dec, date_to_mjd(t.disc_date) + 2400000.5
                    )
                    if TESSFlag:
                        print("tagging %s" % t)
                        try:
                    ...
                    TESSFlag = tess_obs(
                        t.ra, t.dec, date_to_mjd(t.modified_date) + 2400000.5
                    )
                    if TESSFlag:
                        print("tagging %s" % t)
                        try:
