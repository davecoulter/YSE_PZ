...
class ForcedPhot:
	...
	def request(self,snid,ra,dec,diff_id,warp_id,stack_id,width=300,height=300):

		data = at.Table(names=('ROWNUM', 'PROJECT', 'SURVEY_NAME', 'IPP_RELEASE', 'JOB_TYPE', 'OPTION_MASK', 'REQ_TYPE', 'IMG_TYPE', 'ID', 'TESS_ID', 'COMPONENT', 'COORD_MASK', 'CENTER_X', 'CENTER_Y', 'WIDTH', 'HEIGHT', 'DATA_GROUP', 'REQFILT', 'MJD_MIN', 'MJD_MAX', 'RUN_TYPE', 'FWHM_MIN', 'FWHM_MAX', 'COMMENT'))

		skycell_str = 'skycell.'+getskycell(ra,dec)[0]
		
		data.add_row((1,'gpc1','null','null','stamp',2049,'byid','diff',diff_id,'RINGS.V3',skycell_str,2,ra,dec,width,height,'null','null',0,0,'null',0,0,'diff for %s'%snid) )
		data.add_row((1,'gpc1','null','null','stamp',2049,'byid','warp',warp_id,'RINGS.V3',skycell_str,2,ra,dec,width,height,'null','null',0,0,'null',0,0,'warp for %s'%snid) )
		data.add_row((1,'gpc1','null','null','stamp',2049,'byid','stack',stack_id,'RINGS.V3',skycell_str,2,ra,dec,width,height,'null','null',0,0,'null',0,0,'stack for %s'%snid) )
		
		data.write('stamp_request.fits', default_forcedphot_header, format='fits',overwrite=True)
	...
	def add_options(self, parser=None, usage=None, config=None):
		...
		if config:
			...
			parser.add_argument('--skycell', default=config.get('yse_forcedphot','skycell'), type=str,
							  help='stamp upload server (default=%default)')
			parser.add_argument('--user', default=config.get('yse_forcedphot','user'), type=str,
							  help='stamp upload server (default=%default)')
			parser.add_argument('--pass', default=config.get('yse_forcedphot','pass'), type=str,
							  help='stamp upload server (default=%default)')
