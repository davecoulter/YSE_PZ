...
def get_next_name(current_name):
    start = False
    current_str = current_name.replace('YSEStack%s'%datetime.datetime.now().year,'')
    for s in itertools.islice(iter_all_strings(), 20000):
        if start == True:
            next_name = 'YSEStack%s%s'%(datetime.datetime.now().year,s)
            break
        if current_str == s:
            start = True
    return next_name
...
class YSE_Stack(CronJobBase):
    ...
    def main(self):
        ...
        if len(latest_stacked_transients):
            ...
            self.next_name = get_next_name(stacked_name)
        else:
        ...
        # grab CSV files
        r = requests.get(url=self.options.yselink_stacksummary,
                         auth=HTTPBasicAuth(self.options.qubuser,self.options.qubpass))

        if r.status_code != 200: raise RuntimeError('problem accessing summary link %s'%self.options.yselink_stacksummary)
        summary = at.Table.read(r.text, format='ascii', delimiter='|')


        r = requests.get(url=self.options.yselink_stacklc,
                         auth=HTTPBasicAuth(self.options.qubuser,self.options.qubpass))
        if r.status_code != 200: raise RuntimeError('problem accessing lc link %s'%self.options.yselink_stacklc)
        lc = at.Table.read(r.text, format='ascii', delimiter='|')
