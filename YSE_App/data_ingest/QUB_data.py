...
class QUB(CronJobBase):
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        if config:
            ...
            parser.add_argument(
                "--qubuser",
                default=config.get("main", "qubuser"),
                type=str,
                help="QUB database username (default=%default)",
            )
            parser.add_argument(
                "--qubpass",
                default=config.get("main", "qubpass"),
                type=str,
                help="QUB database password (default=%default)",
            )
            parser.add_argument(
                "--psstlink_summary",
                default=config.get("main", "psstlink_summary"),
                type=str,
                help="PSST summary CSV (default=%default)",
            )
            parser.add_argument(
                "--psstlink_lc",
                default=config.get("main", "psstlink_lc"),
                type=str,
            ...
            parser.add_argument(
                "--max_days",
                default=config.get("main", "max_days_qub"),
                type=float,
                help="grab photometry/objects from the last x days",
            )
    ...
    def main(self):
        ...
        # grab CSV files
        r = requests.get(
            url=self.options.psstlink_summary,
            auth=HTTPBasicAuth(self.options.qubuser, self.options.qubpass),
        )
        if r.status_code != 200:
            raise RuntimeError(
                "problem accessing summary link %s" % self.options.yselink_summary
            )
        summary = at.Table.read(r.text, format="ascii.csv", delimiter="|")

        r = requests.get(
            url=self.options.psstlink_lc,
            auth=HTTPBasicAuth(self.options.qubuser, self.options.qubpass),
        )
        if r.status_code != 200:
            raise RuntimeError(
                "problem accessing lc link %s" % self.options.yselink_summary
            )
        ...
        while nsn_single == 25:
            transientdict, nsn_single = self.parse_data(
                summary_upload, lc, transient_idx=nsn, max_transients=25
            )
    ...
    def parse_data(self, summary, lc, transient_idx=0, max_transients=None):
        ...
        for i, s in enumerate(summary[transient_idx : transient_idx + max_transients]):
            ...
            iLC = (lc["ps1_designation"] == s["ps1_designation"]) & (
                nowmjd - lc["mjd_obs"] < self.options.max_days
            )

            if (
                nowmjd
                - Time(
                    "%s 00:00:00" % s["followup_flag_date"], format="iso", scale="utc"
                ).mjd
                > self.options.max_days
            ):
                status = "Ignore"
            else:
            ...
            tdict = {
                "name": s["ps1_designation"],
                "ra": s["ra_psf"],
                "dec": s["dec_psf"],
                "obs_group": "PSST",
                "postage_stamp_file": "%s/%s/%s.jpeg"
                % (
                    s["ps1_designation"],
                    s["target"].split("_")[1].split(".")[0],
                    s["target"],
                ),
                "postage_stamp_ref": "%s/%s/%s.jpeg"
                % (
                    s["ps1_designation"],
                    s["target"].split("_")[1].split(".")[0],
                    s["ref"],
                ),
                "postage_stamp_diff": "%s/%s/%s.jpeg"
                % (
                    s["ps1_designation"],
                    s["target"].split("_")[1].split(".")[0],
                    s["diff"],
                ),
                "postage_stamp_file_fits": "%s/%s/%s.fits"
                % (
                    s["ps1_designation"],
                    s["target"].split("_")[1].split(".")[0],
                    s["target"],
                ),
                "postage_stamp_ref_fits": "%s/%s/%s.fits"
                % (
                    s["ps1_designation"],
                    s["target"].split("_")[1].split(".")[0],
                    s["ref"],
                ),
                "postage_stamp_diff_fits": "%s/%s/%s.fits"
                % (
                    s["ps1_designation"],
                    s["target"].split("_")[1].split(".")[0],
                    s["diff"],
            ...
            PhotUploadAll = {"mjdmatchmin": 0.01, "clobber": self.options.clobber}
            photometrydict = {"instrument": "GPC1", "obs_group": "PSST", "photdata": {}}
            for j, l in enumerate(lc[iLC][np.argsort(lc["mjd_obs"][iLC])]):
                if (
                    j == 0
                    and np.abs(date_to_mjd(s["followup_flag_date"]) - l["mjd_obs"]) < 1
                ):
                    disc_point = 1
                else:
                    disc_point = 0
                ...
                phot_upload_dict = {
                    "obs_date": mjd_to_date(l["mjd_obs"]),
                    "band": l["filter"],
                    "groups": [],
                    "mag": l["cal_psf_mag"],
                    "mag_err": mag_err,
    ...
    def getZTFPhotometry(self, ra, dec):
        ...
        ztfurl = (
            "%s/?format=json&sort_value=jd&sort_order=desc&cone=%.7f%%2C%.7f%%2C0.0014"
    ...
    def copy_stamps(self, transientdict):
        try:
            if not djangoSettings.DEBUG:
                basedir = "%sYSE_App/images/stamps" % (djangoSettings.STATIC_ROOT)
            else:
                basedir = "%s/YSE_App%sYSE_App/images/stamps" % (
                    djangoSettings.BASE_DIR,
                    self.options.STATIC,
                )
            for k in transientdict.keys():
                ...
                if not os.path.exists(
                    "%s/%s"
                    % (basedir, os.path.dirname(transientdict[k]["postage_stamp_ref"]))
                ):
                    os.makedirs(
                        "%s/%s"
                        % (
                            basedir,
                            os.path.dirname(transientdict[k]["postage_stamp_ref"]),
                        )
                    )
                    ...
                    if not os.path.exists(
                        "%s/%s" % (basedir, transientdict[k]["postage_stamp_ref"])
                    ):
                        r = requests.get(
                            "%s/%s"
                            % (
                                psst_image_url,
                                transientdict[k]["postage_stamp_ref"].split("/", 1)[-1],
                            ),
                            auth=HTTPBasicAuth(
                                self.options.qubuser, self.options.qubpass
                            ),
                        )
                        with iopen(
                            "%s/%s" % (basedir, transientdict[k]["postage_stamp_ref"]),
                    ...
                    if not os.path.exists(
                        "%s/%s" % (basedir, transientdict[k]["postage_stamp_diff"])
                    ):
                        r = requests.get(
                            "%s/%s"
                            % (
                                psst_image_url,
                                transientdict[k]["postage_stamp_diff"].split("/", 1)[
                                    -1
                                ],
                            ),
                            auth=HTTPBasicAuth(
                    ...
                    if not os.path.exists(
                        "%s/%s" % (basedir, transientdict[k]["postage_stamp_file"])
                    ):
                        r = requests.get(
                            "%s/%s"
                            % (
                                psst_image_url,
                                transientdict[k]["postage_stamp_file"].split("/", 1)[
                                    -1
                                ],
                            ),
                            auth=HTTPBasicAuth(
                    ...
                    if not os.path.exists(
                        "%s/%s" % (basedir, transientdict[k]["postage_stamp_ref_fits"])
                    ):
                        r = requests.get(
                            "%s/%s"
                            % (
                                psst_image_url,
                                transientdict[k]["postage_stamp_ref_fits"].split(
                                    "/", 1
                                )[-1],
                            ),
                            auth=HTTPBasicAuth(
                        ...
                        with iopen(
                            "%s/%s"
                            % (basedir, transientdict[k]["postage_stamp_ref_fits"]),
                            "wb",
                        ) as fout:
                            fout.write(r.content)

                    if not os.path.exists(
                        "%s/%s" % (basedir, transientdict[k]["postage_stamp_diff_fits"])
                    ):
                        r = requests.get(
                            "%s/%s"
                            % (
                                psst_image_url,
                                transientdict[k]["postage_stamp_diff_fits"].split(
                                    "/", 1
                                )[-1],
                            ),
                            auth=HTTPBasicAuth(
                        ...
                        with iopen(
                            "%s/%s"
                            % (basedir, transientdict[k]["postage_stamp_diff_fits"]),
                            "wb",
                        ) as fout:
                            fout.write(r.content)

                    if not os.path.exists(
                        "%s/%s" % (basedir, transientdict[k]["postage_stamp_file_fits"])
                    ):
                        r = requests.get(
                            "%s/%s"
                            % (
                                psst_image_url,
                                transientdict[k]["postage_stamp_file_fits"].split(
                                    "/", 1
                                )[-1],
                            ),
                            auth=HTTPBasicAuth(
                        ...
                        with iopen(
                            "%s/%s"
                            % (basedir, transientdict[k]["postage_stamp_file_fits"]),
                            "wb",
                        ) as fout:
                            fout.write(r.content)
...
class YSE(CronJobBase):
    ...
    def do(self):
        ...
        try:
            ...
            print("Sending error email")
            html_msg = (
                "Alert : YSE_PZ Failed to upload transients from YSE in QUB_data.py\n"
            )
            html_msg += "Error : %s"
            sendemail(
                from_addr,
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        if config:
            ...
            parser.add_argument(
                "--yselink_summary",
                default=config.get("main", "yselink_summary"),
                type=str,
                help="YSE summary CSV (default=%default)",
            )
            parser.add_argument(
                "--yselink_lc",
                default=config.get("main", "yselink_lc"),
                type=str,
                help="YSE lightcurve CSV (default=%default)",
            )
            parser.add_argument(
                "--yselink_genericsummary",
                default=config.get("main", "yselink_genericsummary"),
                type=str,
                help="YSE summary CSV for possible candidates (default=%default)",
            )
            parser.add_argument(
                "--yselink_genericlc",
                default=config.get("main", "yselink_genericlc"),
                type=str,
            ...
            parser.add_argument(
                "--max_days",
                default=config.get("main", "max_days_yse"),
                type=float,
                help="grab photometry/objects from the last x days",
            )
    ...
    def main(self):
        ...
        r = requests.get(
            "%s/query_api/%s/" % (urlbase, urllib.parse.quote(querytitle)),
            auth=HTTPBasicAuth(self.options.dblogin, self.options.dbpassword),
        )
        data = json.loads(r.text)

        nsn = 0
        for yselink_summary, yselink_lc, namecol in zip(
            [self.options.yselink_summary, self.options.yselink_genericsummary],
            ...
            iSummary = np.append(
                iSummary,
                np.where(
                    (nowmjd - summary["mjd_obs"] < self.options.max_days)
                    | (nowmjd - summary["latest_mjd_forced"] < self.options.max_days)
                )[0],
            )
            ...
            while nsn_single == 10:
                transientdict, nsn_single = self.parse_data(
                    summary_upload, lc, transient_idx=nsn, max_transients=10
                )
                print("uploading %i transients" % nsn_single)
                self.send_data(transientdict)
                self.copy_stamps(transientdict)
    ...
    def parse_data(self, summary, lc, transient_idx=0, max_transients=None):
        ...
        for i, s in enumerate(summary[transient_idx : transient_idx + max_transients]):
            ...
            r = requests.get(
                url="https://psweb.mp.qub.ac.uk/sne/ps1yse/psdb/lightcurveforced/%s"
                % s["id"]
            )
            ...
            iLC = (lc["local_designation"] == s["local_designation"]) & (
                nowmjd - lc["mjd_obs"] < self.options.max_days
            )
            ...
            tdict = {
                ...
                "status": status,
                "context_class": s["sherlockClassification"].replace(
                    "UNCLEAR", "Unknown"
                ),
                "host": hostdict,
                "tags": ["YSE"],
            ...
            if has_forced_phot:
                for j, lf in enumerate(lc_forced[np.argsort(lc_forced["mjd"])]):
                    if (
                        j == 0
                        and np.abs(date_to_mjd(s["followup_flag_date"]) - lf["mjd"]) < 1
                    ):
                        disc_point = 1
                    else:
                        disc_point = 0
                    ...
                    # forced photometry
                    phot_upload_dict = {
                        "obs_date": mjd_to_date(lf["mjd"]),
                        "band": lf["filter"],
                        "groups": ["YSE"],
                        "mag": forced_mag,
                        "mag_err": forced_mag_err,
                        "flux": fluxToMicroJansky(
                            lf["psf_inst_flux"], 27.0, lf["zero_pt"]
                        )
                        * 10 ** (0.4 * (27.5 - 23.9)),
                        "flux_err": fluxToMicroJansky(
                            lf["psf_inst_flux_sig"], 27.0, lf["zero_pt"]
                        )
                        * 10 ** (0.4 * (27.5 - 23.9)),
                        "data_quality": 0,
                        "forced": 1,
                    ...
                    if lf["pscamera"] == "GPC1":
                        gpc1photometrydict["photdata"][
                            "%s_%i" % (mjd_to_date(lf["mjd"]), j)
                        ] = phot_upload_dict
                    elif lf["pscamera"] == "GPC2":
                        gpc2photometrydict["photdata"][
                            "%s_%i" % (mjd_to_date(lf["mjd"]), j)
                        ] = phot_upload_dict
                    else:
                        raise RuntimeError(f"unknown camera! {lf['pscamera']}")

            for j, l in enumerate(lc[iLC][np.argsort(lc["mjd_obs"][iLC])]):
                if (
                    has_forced_phot
                    and np.min(np.abs(lc_forced["mjd"] - l["mjd_obs"])) < 0.01
                ):
                    continue
                if (
                    j == 0
...
class YSE_Stack(CronJobBase):
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        if config:
            ...
            parser.add_argument(
                "--yselink_stacksummary",
                default=config.get("main", "yselink_stacksummary"),
                type=str,
                help="YSE summary CSV for possible candidates (default=%default)",
            )
            parser.add_argument(
                "--yselink_stacklc",
                default=config.get("main", "yselink_stacklc"),
                type=str,
                help="YSE lightcurve CSV for possible candidates (default=%default)",
            )
            parser.add_argument(
                "--yselink_agnsummary",
                default=config.get("main", "yselink_agnsummary"),
                type=str,
                help="YSE summary CSV for possible candidates (default=%default)",
            )
            parser.add_argument(
                "--yselink_agnlc",
                default=config.get("main", "yselink_agnlc"),
                type=str,
                help="YSE lightcurve CSV for possible candidates (default=%default)",
            )
            parser.add_argument(
            ...
            parser.add_argument(
                "--max_days_ysestacklc",
                default=config.get("main", "max_days_ysestacklc"),
                type=float,
                help="maximum days to look back for lightcurves (default=%default)",
            )
            parser.add_argument(
                "--max_days_yseignore",
                default=config.get("main", "max_days_ysestackignore"),
                type=float,
                help="maximum days to look back for lightcurves (default=%default)",
            )
            parser.add_argument(
                "--max_days_yseagnlc",
                default=config.get("main", "max_days_yseagnlc"),
                type=float,
                help="maximum days to look back for lightcurves (default=%default)",
            )
            parser.add_argument(
                "--max_days_yseagnignore",
                default=config.get("main", "max_days_yseagnignore"),
                type=float,
                help="maximum days to look back for lightcurves (default=%default)",
            )
    ...
    def main(self):
        ...
        # grab CSV files
        for summarylink, lclink, naming in zip(
            [self.options.yselink_stacksummary, self.options.yselink_agnsummary],
            [self.options.yselink_stacklc, self.options.yselink_agnlc],
            ["stack", "agn"],
        ):

            if naming == "stack":
                latest_stacked_transients = Transient.objects.filter(
                    tags__name="YSE Stack"
                ...
                if len(latest_stacked_transients):
                    ...
                    self.next_name = get_next_name(
                        stacked_name, ttype="Stack", year=year
                    )
                else:
                    self.next_name = "YSEStack%sa" % year
            else:
                latest_agn = Transient.objects.filter(tags__name="YSE AGN").order_by(
                    "-created_date"
                )
            ...
            if r.status_code != 200:
                raise RuntimeError("problem accessing summary link %s" % summarylink)
            summary = at.Table.read(r.text, format="ascii", delimiter="|")
            ...
            if r.status_code != 200:
                raise RuntimeError("problem accessing lc link %s" % lclink)
            lc = at.Table.read(r.text, format="ascii", delimiter="|")
            ...
            nowmjd = Time.now().mjd
            if naming == "stack":
                summary_upload = summary[
                    nowmjd - summary["mjd_obs"] < self.options.max_days_ysestacklc
                ]
            else:
                summary_upload = summary[
                    nowmjd - summary["mjd_obs"] < self.options.max_days_yseagnlc
                ]
            # important to sort by flag date for a halfway sensible naming scheme
            summary_upload = summary_upload[
                np.argsort(date_to_mjd(summary_upload["followup_flag_date"]))
            ]
    ...
    def parse_data(
        ...
        if naming_convention == "stack":
            ysestacktag = TransientTag.objects.get(name="YSE Stack")
        elif naming_convention == "agn":
            ysestacktag = TransientTag.objects.get(name="YSE AGN")
        else:
            raise RuntimeError("unknown naming convention")
        for i, s in enumerate(summary[transient_idx : transient_idx + max_transients]):
            # check RA/dec
            ramin, ramax, decmin, decmax = getRADecBox(
                s["ra_psf"], s["dec_psf"], size=0.00042
            )
            dbtransient = Transient.objects.filter(
                ...
                & Q(dec__lt=decmax)
                & Q(
                    disc_date__gte=dateutil.parser.parse(s["followup_flag_date"])
                    - datetime.timedelta(365)
                )
                & Q(
                    disc_date__lte=dateutil.parser.parse(s["followup_flag_date"])
                    + datetime.timedelta(365)
                )
            ...
            if len(dbstacktransient):
                stackname = dbstacktransient[0].name
            else:
                stackname = None
            ...
            if naming_convention == "stack":
                ...
                r = requests.get(
                    url="https://psweb.mp.qub.ac.uk/sne/ps1ysestack/psdb/lightcurveforced/%s"
                    % s["id"]
                )
            else:
                # r = requests.get(url='https://star.pst.qub.ac.uk/sne/ps1yse/psdb/lightcurveforced/%s'%s['id'])
            ...
            if r.status_code != 200:
                raise RuntimeError("problem accessing forcedphot link %s")
            try:
                lc_forced = at.Table.read(r.text, format="ascii", delimiter=" ")
                if len(lc_forced):
                    has_forced_phot = True
            ...
            mw_ebv = float("%.3f" % (sfd(sc) * 0.86))
            iLC = (
                lc["transient_object_id"] == s["id"]
            )  # & (nowmjd - lc['mjd_obs'] < self.options.max_days_ysestacklc)

            if naming_convention == "stack":
                if (
                    nowmjd
                    - Time(
                        "%s 00:00:00" % s["followup_flag_date"],
                        format="iso",
                        scale="utc",
                    ).mjd
                    > self.options.max_days_yseignore
                ):
                    status = "Ignore"
                else:
                    status = self.options.status
                ...
                if (
                    ...
                    > self.options.max_days_yseagnignore
                ):
                    status = "Ignore"
                else:
                    status = self.options.status
            ...
            if stackname:
                ...
                if naming_convention == "stack":
                    if str(year) not in self.next_name:
                        self.next_name = "YSEStack%sa" % str(year)
                    else:
                        self.next_name = get_next_name(
                            self.next_name, ttype="Stack", year=year
                        )
                else:
                    if str(year) not in self.next_name:
                        self.next_name = "YSEAGN%sa" % str(year)
                    else:
                        self.next_name = get_next_name(
                            self.next_name, ttype="AGN", year=year
                        )
            ...
            PhotUploadAll = {"mjdmatchmin": 0.01, "clobber": self.options.clobber}
            photometrydict = {"instrument": "GPC1", "obs_group": "YSE", "photdata": {}}
            if has_forced_phot:
                for j, lf in enumerate(lc_forced[np.argsort(lc_forced["mjd"])]):
                    if (
                        j == 0
...
class CheckDuplicates(CronJobBase):
    ...
    def do(self):
        ...
        print(
            "starting duplicate check at {}".format(datetime.datetime.now().isoformat())
        )
        ...
        print(
            "Checking duplicates took %.1f seconds for %i transients"
            % (time.time() - tstart, nsn)
        )
    ...
    def main(self):
        ...
        transients = Transient.objects.filter(
            created_date__gt=datetime.datetime.now() - datetime.timedelta(1)
        ).filter(name__startswith="10")
        for t in transients:
            ramin, ramax, decmin, decmax = getRADecBox(t.ra, t.dec, size=0.00042)
            dups = Transient.objects.filter(
                Q(ra__gt=ramin)
                & Q(ra__lt=ramax)
                & Q(dec__gt=decmin)
                & Q(dec__lt=decmax)
...
def get(url, json_list, api_key, tns_user_id, tns_user_name):
    try:
        ...
        # headers
        headers = {
            "User-Agent": 'tns_marker{"tns_id":' + str(tns_user_id) + ', "type":"user",'
            ' "name":"' + tns_user_name + '"}'
        }
