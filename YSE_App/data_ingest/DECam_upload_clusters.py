...
def get_candidate_photometry(candnum, clusterfile, date):
    ...
    forced_file = '{0}_{1}.{2}/{0}_{1}.{2}_cand{3}.forced.difflc.txt'.format(field, chip,
        num, candnum)
        ...
        for mydate in date_list:
            ...
            for clusterfile in clusters:
                ...
                for row in clustersdata['candidates']:
                    if row['ysename']!='-':
                        lcdata=get_candidate_photometry(row['ID'], clusterfile, mydate.isoformat())

                        if not lcdata: continue
