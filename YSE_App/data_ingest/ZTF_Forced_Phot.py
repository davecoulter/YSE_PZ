...
warnings.filterwarnings(
    "ignore"
)  # We'll get warnings from log10 when there are non-detections
...
def random_log_file_name(log_file_dir="/tmp"):
    ...
    while log_file_name is None or os.path.exists(log_file_name):
        log_file_name = f"{log_file_dir}/forced_phot_out/ztffp_%s.txt" % "".join(
...
def read_job_log(file_name):
    ...
    job_info["dec"] = np.format_float_positional(
        float(job_info["dec"].to_list()[0]), precision=6, pad_right=6
    ).replace(" ", "0")
    job_info["jdstart"] = np.format_float_positional(
        float(job_info["jdstart"].to_list()[0]), precision=6, pad_right=6
    ).replace(" ", "0")
...
class ZTF_Forced_Phot:
    def __init__(
        self,
        ztf_email_address=None,
        ztf_email_password=None,
        ztf_email_imapserver=None,
        ztf_user_address=None,
        ...
        #
        if ztf_email_address is None:
            self._ztffp_email_address = os.environ["ztf_email_address"]
        else:
            self._ztffp_email_address = ztf_email_address

        if ztf_email_password is None:
            self._ztffp_email_password = os.environ["ztf_email_password"]
        else:
            self._ztffp_email_password = ztf_email_password

        if ztf_email_imapserver is None:
            self._ztffp_email_server = os.environ["ztf_email_imapserver"]
        else:
            self._ztffp_email_server = ztf_email_imapserver

        if ztf_user_address is None:
            self._ztffp_user_address = os.environ["ztf_user_address"]
        else:
            self._ztffp_user_address = ztf_user_address

        if ztf_user_password is None:
            self._ztffp_user_password = os.environ["ztf_user_password"]
        else:
            self._ztffp_user_password = ztf_user_password
            ...
            for i in range(int(messages[0]), 0, -1)[0:100]:
                ...
                for response in msg:
                    if isinstance(response, tuple):
                        ...
                        if not isinstance(sender, bytes) and re.search(
                            ...
                            if content_type == "text/plain":
                                processing_match = match_ztf_message(
                                ...
                                if processing_match:
                                    ...
                                    lc_url = (
                                        "https"
                                        + (body.split("_lc.txt")[0] + "_lc.txt").split(
                                            "https"
                                        )[-1]
                                    )
                                    log_url = (
                                        "https"
                                        + (
                                            body.split("_log.txt")[0] + "_log.txt"
                                        ).split("https")[-1]
                                    )

                                    # Download each file
                                    lc_initial_file_name = download_ztf_url(
                                        lc_url, verbose=verbose
                                    )
                                    log_initial_file_name = download_ztf_url(
                                        log_url, verbose=verbose
                                    )

                                    # Rename
                                    if source_name is None:
                                        downloaded_file_names = [
                                            lc_initial_file_name,
    ...
    def ztf_forced_photometry(
        self, ra, decl, jdstart=None, jdend=None, days=60, send=True, verbose=True
    ):
        ...
        if ra is not None and decl is not None:
            ...
            log_file_name = random_log_file_name(
                log_file_dir=djangoSettings.ZTFTMPDIR
            )  # Unique file name
            ...
            wget_command = (
                ...
                % (_ztfuser, _ztfinfo, log_file_name)
                + "ra=%s&" % ra_str
                + "dec=%s&" % decl_str
    ...
    def plot_ztf_fp(
        ...
        try:
            ...
            if verbose:
                print(
                    "Empty ZTF light curve file (%s). Check the log file."
                    % lc_file_name
                )
            return
        ...
        #
        ztf_fp_df["mjd_midpoint"] = (
            ztf_fp_df["jd"] - 2_400_000.5 - ztf_fp_df["exptime"] / 2.0 / 86400.0
        )  # Do it all at once here
        ztf_fp_df["fp_mag"] = ztf_fp_df["zpdiff"] - 2.5 * np.log10(
            ztf_fp_df["forcediffimflux"]
        )
        ztf_fp_df["fp_mag_unc"] = (
            1.0857 * ztf_fp_df["forcediffimfluxunc"] / ztf_fp_df["forcediffimflux"]
        )
        ztf_fp_df["fp_ul"] = ztf_fp_df["zpdiff"] - 2.5 * np.log10(
            upperlimit * ztf_fp_df["forcediffimfluxunc"]
        )
        ...
        for ztf_filter in set(ztf_fp_df["filter"]):
            ...
            # Upper limit df
            ul_filter_df = filter_df[
                filter_df.forcediffimflux / filter_df.forcediffimfluxunc < threshold
            ]

            # Detections df
            detection_filter_df = filter_df[
                filter_df.forcediffimflux / filter_df.forcediffimfluxunc >= threshold
            ]

            if verbose:
                print(
                    "%s: %s detections and %s upper limits."
                    % (ztf_filter, detection_filter_df.shape[0], ul_filter_df.shape[0])
                )

            # Plot detections
            plt.plot(
                detection_filter_df.mjd_midpoint,
                detection_filter_df.fp_mag,
                color=filter_colors[ztf_filter],
                marker="o",
                linestyle="",
                zorder=3,
            )
            plt.errorbar(
                detection_filter_df.mjd_midpoint,
                detection_filter_df.fp_mag,
                yerr=detection_filter_df.fp_mag_unc,
                color=filter_colors[ztf_filter],
                linestyle="",
                zorder=1,
            )
    ...
    def get_ztf_fp(
        self, log_file_name, directory_path=".", source_name="temp", verbose=False
    ):
        ...
        downloaded_file_names = self.query_ztf_email(
            log_file_name, source_name=source_name, verbose=verbose
        )
            ...
            for downloaded_file_name in downloaded_file_names:
                if os.path.exists(downloaded_file_name):
                    shutil.move(
                        ...
                        "%s/%s" % (output_directory, downloaded_file_name),
                    )

                    if verbose:
                        print(
                            "%sZTF downloaded file: %s/%s"
                            % (" " * 5, output_directory, downloaded_file_name)
                        )
                    output_files.append(
                        "%s/%s" % (output_directory, downloaded_file_name)
                    )
    ...
    def run_ztf_fp(
        ...
        run = False
        if (
            (ra is not None and decl is not None)
            or (logfile is not None)
            or (plotfile is not None)
        ):
            run = True
        ...
        if mjdstart is not None:
            jdstart = mjdstart + 2_400_000.5
        if mjdend is not None:
            jdend = mjdend + 2_400_000.5
        ...
        if logfile is None and plotfile is None:
            ...
            log_file_name = self.ztf_forced_photometry(
                ra=ra, decl=decl, jdstart=jdstart, jdend=jdend, days=days
            )
            ...
            if emailcheck > 0:
                while downloaded_file_names is None:
                    ...
                    if time.time() - time_start_seconds < emailcheck:
                        if verbose:
                            print(
                                "Waiting for the email (rechecking every %s seconds)."
                                % emailcheck
                            )
                    ...
                    if downloaded_file_names == -1:
                        ...
                        if (
                            ...
                            and time.time() - time_start_seconds > 600
                        ):  # After 5 minutes, change to checking every 1 minute
                            emailcheck = fivemindelay
                            if verbose:
                                print(
                                    "Changing to re-checking every %s seconds."
                                    % emailcheck
                                )
                ...
                shutil.move(
                    log_file_name,
                    "%s/%s" % (output_directory, log_file_name.split("/")[-1]),
                )
        ...
        if downloaded_file_names is not None and downloaded_file_names[0] is not None:
            ...
            if do_plot:
                figure_file_name = plot_ztf_fp(
                    downloaded_file_names[0], verbose=verbose
                )
            else:
        ...
        if skip_clean == False:
            ...
            if log_file_name is not None and os.path.exists(log_file_name):
                ...
                if verbose:
                    print(
                        "%sZTF wget log: %s/%s"
                        % (" " * 5, output_directory, log_file_name)
                    )
                output_files.append("%s/%s" % (output_directory, log_file_name))
            ...
            if figure_file_name is not None and os.path.exists(figure_file_name):
                shutil.move(
                    figure_file_name, "%s/%s" % (output_directory, figure_file_name)
                )
                if verbose:
                    print(
                        "%sZTF figure: %s/%s"
                        % (" " * 5, output_directory, figure_file_name)
                    )
