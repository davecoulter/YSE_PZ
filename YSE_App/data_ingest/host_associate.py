        ...
        try:
            ...
            for i, T in enumerate(transients):
                if T.host.redshift is not None or T.redshift is not None:
                    ...
                    if (
                        np.arccos(
                            np.sin(t_dec) * np.sin(h_dec)
                            + np.cos(t_dec) * np.cos(h_dec) * np.cos(abs(t_ra - h_ra))
                        )
                        * (3e5 * redshift / 73)
                        / (pow(1.0 + redshift, 2) * 1000)
                    ) < 80:
                        mask.append(False)
                    else:
                        mask.append(True)
            ...
            hosts = getTransientHosts(
                snName, snCoord, verbose=True, starcut="normal", ascentMatch=False
            )
            ...
            for i, T in enumerate(transients):
                ...
                T.host.ra = hosts[hosts["TransientName"] == "SN" + str(i)][
                    "raMean"
                ].values[0]
                T.host.dec = hosts[hosts["TransientName"] == "SN" + str(i)][
                    "decMean"
                ].values[0]
                T.host.panstarrs_objid = hosts[hosts["TransientName"] == "SN" + str(i)][
                    "objID"
                ].values[0]
                T.host.save()
            ...
            print(
                """GHOST cron failed with error %s at line number %s"""
                % (e, exc_tb.tb_lineno)
            )
