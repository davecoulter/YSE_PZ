...
class SurveyObs(CronJobBase):
    ...
    def do(self):
        ...
        try:
            ...
            if os.path.exists("%s/surveyfields.txt" % djangoSettings.STATIC_ROOT):
                self.add_survey_fields(
                    "%s/surveyfields.txt" % djangoSettings.STATIC_ROOT
                )
    ...
    def add_survey_fields(self, surveyfile):
        ...
        r = requests.post(
            url=url,
            data=json.dumps(SurveyUploadDict),
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        if config:
            ...
            parser.add_argument(
                "--SMTP_LOGIN",
                default=config.get("YSE_SMTP_provider", "SMTP_LOGIN"),
                type=str,
                help="SMTP login (default=%default)",
            )
            parser.add_argument(
                "--SMTP_PASSWORD",
                default=config.get("YSE_SMTP_provider", "SMTP_PASSWORD"),
                type=str,
                help="SMTP password (default=%default)",
            )
            parser.add_argument(
                "--SMTP_HOST",
                default=config.get("YSE_SMTP_provider", "SMTP_HOST"),
                type=str,
                help="SMTP host (default=%default)",
            )
            parser.add_argument(
                "--SMTP_PORT",
                default=config.get("YSE_SMTP_provider", "SMTP_PORT"),
                type=str,
