...
def fits_to_png(ff, outfile, log=False):
    ...
    circle = plt.Circle(
        (np.shape(fim)[0] / 2 - 1, np.shape(fim)[1] / 2 - 1), 15, color="r", fill=False
    )
...
class ForcedPhot(CronJobBase):
    ...
    def do(self):
        ...
        try:
            ...
            print(
                """Forced phot cron failed with error %s at line number %s"""
                % (e, exc_tb.tb_lineno)
            )
            nsn = 0
        ...
        print(
            "YSE_PZ Forced Photometry took %.1f seconds for %i transients"
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        if config:
            ...
            parser.add_argument(
                "--upload",
                default=config.get("yse_forcedphot", "upload"),
                type=str,
                help="stamp upload server (default=%default)",
            )
            parser.add_argument(
                "--stamp",
                default=config.get("yse_forcedphot", "stamp"),
                type=str,
                help="stamp upload server (default=%default)",
            )
            parser.add_argument(
                "--detectability",
                default=config.get("yse_forcedphot", "detectability"),
                type=str,
                help="stamp upload server (default=%default)",
            )
            parser.add_argument(
                "--skycell",
                default=config.get("yse_forcedphot", "skycell"),
                type=str,
                help="stamp upload server (default=%default)",
            )
            parser.add_argument(
                "--ifauser",
                default=config.get("yse_forcedphot", "user"),
                type=str,
                help="stamp upload server (default=%default)",
            )
            parser.add_argument(
                "--ifapass",
                default=config.get("yse_forcedphot", "pass"),
                type=str,
                help="stamp upload server (default=%default)",
            )
    ...
    def doall_main(self):
        ...
        transients = (
            Transient.objects.filter(created_date__gte=min_date)
            .filter(~Q(tags__name="YSE") & ~Q(tags__name="YSE Stack"))
    ...
    def main(self, transient_name=None, update_forced=False):
        ...
        if transient_name is None and not update_forced:
            ...
            min_date_forcedphot = datetime.datetime.utcnow() - datetime.timedelta(
                days=7
            )
            transients = Transient.objects.filter(
                ...
                & Q(
                    transientphotometry__transientphotdata__obs_date__gt=min_date_forcedphot
                )
                & Q(
                    disc_date__gt=datetime.datetime.utcnow()
        ...
        survey_images = (
            SurveyObservation.objects.filter(status__name="Successful")
            .filter(obs_mjd__gt=nowmjd - self.options.max_days_yseimage)
            .filter(diff_id__isnull=False)
        )
        ...
        while not jobs_done and time.time() - tstart < 600:
            ...
            for phot_request_name in phot_request_names:
                ...
                if not done_phot:
                    doneall_phot = False
            if done_stamp and doneall_phot:
                jobs_done = True
        ...
        # save to data model
        phot_dict = self.get_phot(
            phot_request_names, transient_list, ra_list, dec_list, img_dict
        )
        ...
        for t in phot_dict.keys():
            for s, r, d in zip(
                phot_dict[t]["stack_id"], phot_dict[t]["ra"], phot_dict[t]["dec"]
            ):
                stack_id_list += [s]
                ra_list += [r]
                dec_list += [d]
    ...
    def get_images(self, img_dict):
        ...
        for k in img_dict.keys():
            for img_key_in, img_key_out in zip(
                ["warp_image_link", "diff_image_link", "stack_image_link"],
                ...
                for i in range(len(img_dict[k][img_key_in])):
                    ...
                    outdir = "%s/%s/%i" % (
                        ...
                        int(
                            float(
                                img_dict[k]["%s_mjd" % img_key_in.replace("_link", "")][
                                    i
                                ]
                            )
                        ),
                    ...
                    if "diff" in img_key_in:
                        fits_to_png(ff, outfile.replace("fits", "png"), log=False)
                    else:
                        fits_to_png(ff, outfile.replace("fits", "png"), log=True)
                    img_dict[k][img_key_out] += [
                        outfile.replace("{}/".format(basedir), "")
                    ]
                    img_dict[k][img_key_out + "_png"] += [
                        outfile.replace("{}/".format(basedir), "").replace(
                            ".fits", ".png"
                        )
                    ]
    ...
    def write_to_db(self, phot_dict, img_dict, stack_img_dict):
        ...
        for k in img_dict.keys():
            ...
            PhotUploadAll = {"mjdmatchmin": 0.0001, "clobber": True}
            photometrydict = {"instrument": "GPC2", "obs_group": "YSE", "photdata": {}}
            ...
            for i in range(len(img_dict[k]["diff_image_id"])):
                ...
                if img_dict[k]["diff_image_link"][i] is None:
                    valid_pixels = False
                else:
                    valid_pixels = True
                diffimgdict = {
                    "postage_stamp_file": img_dict[k]["warp_file_png"][i],
                ...
                if k in phot_dict.keys():
                    idx = np.where(
                        img_dict[k]["diff_image_id"][i]
                        == np.array(phot_dict[k]["diff_id"])
                    )[0]
                else:
                    idx = None
                if idx is not None and len(idx) and k in phot_dict.keys():
                    ...
                    flux = fluxToMicroJansky(
                        phot_dict[k]["flux"][idx],
                        phot_dict[k]["exptime"][idx],
                        phot_dict[k]["zpt"][idx],
                    ) * 10 ** (0.4 * (27.5 - 23.9))
                    flux_err = fluxToMicroJansky(
                        phot_dict[k]["flux_err"][idx],
                        phot_dict[k]["exptime"][idx],
                        phot_dict[k]["zpt"][idx],
                    ) * 10 ** (0.4 * (27.5 - 23.9))
                    ...
                    phot_upload_dict = {
                        "obs_date": mjd_to_date(phot_dict[k]["mjd"][idx]),
                    ...
                    phot_upload_dict = {
                        "obs_date": mjd_to_date(img_dict[k]["diff_image_mjd"][i]),
                ...
                photometrydict["photdata"][
                    "%s_%i" % (mjd_to_date(img_dict[k]["diff_image_mjd"][i]), i)
                ] = phot_upload_dict
    ...
    def get_phot(
        ...
        for request_name in request_names:
            ...
            if phot_page.status_code != 200:
                raise RuntimeError(
                    "results page {} does not exist".format(phot_results_link)
                )
            ...
            for f in fitsfiles:
                if "detectability" in f:
                    ...
                    for i in range(len(ff[1].data)):
                        ...
                        if (
                            ff[1].data["PSF_QF"][i] < 0.9
                            or (ff[1].data["FLAGS"][i] & 0x00001000)
                            or (ff[1].data["FLAGS"][i] & 0x20000000)
                            or (ff[1].data["FLAGS"][i] & 0x40000000)
                            or (ff[1].data["FLAGS"][i] & 0x80000000)
                            or (ff[1].data["FLAGS"][i] & 0x00000080)
                            or (ff[1].data["FLAGS"][i] & 0x00000800)
                        ):
                            dq = 1
                        else:
                            dq = 0
    ...
    def get_stamps(self, request_name, transient_list):
        ...
        if stamps_page.status_code != 200:
            raise RuntimeError(
                "results page {} does not exist".format(stamp_results_link)
            )
        ...
        for k in mdc_stamps.keys():
            if (
                "SUCCESS" not in mdc_stamps[k]["ERROR_STR"]
                and "NO_VALID_PIXELS" not in mdc_stamps[k]["ERROR_STR"]
                and "PSTAMP_NO_IMAGE_MATCH" not in mdc_stamps[k]["ERROR_STR"]
            ):
                raise RuntimeError("part of job {} failed!".format(request_name))
            img_name, img_type, transient, mjd, img_id, img_filter = (
                mdc_stamps[k]["IMG_NAME"],
                mdc_stamps[k]["IMG_TYPE"],
                mdc_stamps[k]["COMMENT"].split(".")[-1],
                float(mdc_stamps[k]["MJD_OBS"]),
                mdc_stamps[k]["ID"],
                mdc_stamps[k]["FILTER"].split(".")[0],
            )
            if transient not in image_dict.keys():
                image_dict[transient] = {
                    "warp_image_link": [],
                    "diff_image_link": [],
                    "stack_image_link": [],
                    "warp_image_id": [],
            ...
            if (
                "NO_VALID_PIXELS" not in mdc_stamps[k]["ERROR_STR"]
                and "PSTAMP_NO_IMAGE_MATCH" not in mdc_stamps[k]["ERROR_STR"]
            ):
                if img_type == "warp":
                    image_dict[transient]["warp_image_link"] += [
                        "{}/{}".format(stamp_fitsfile_link, img_name)
                    ]
                    ...
                    image_dict[transient]["diff_image_link"] += [
                        "{}/{}".format(stamp_fitsfile_link, img_name)
                    ]
                    ...
                    image_dict[transient]["stack_image_link"] += [
                        "{}/{}".format(stamp_fitsfile_link, img_name)
                    ]
                    ...
                    raise RuntimeError("image type {} not found".format(img_type))
            else:
                if img_type == "warp":
    ...
    def get_status(self, request_name):
        ...
        if page.status_code == 200:
            ...
            text = "\n".join(lines_out)
            tbl = at.Table.read(
                text, format="ascii", delimiter="|", data_start=1, header_start=0
            )
            ...
            if not len(tbl[idx]):
                ...
                return False, False
            if tbl["Completion Time (UTC)"][idx]:
                done = True
            else:
                done = False

            if float(tbl["Total Jobs"][idx]) == float(tbl["Successful Jobs"][idx]):
                success = True
            else:
                success = False
                print(
                    "warning: %i of %i jobs failed"
                    % (
                        float(tbl["Total Jobs"][idx])
                        - float(tbl["Successful Jobs"][idx]),
                        float(tbl["Total Jobs"][idx]),
                    )
                )
    ...
    def stamp_request(
        ...
        assert (
            len(transient_list) == len(warp_id_list)
            or len(transient_list) == len(diff_id_list)
            or len(transient_list) == len(stack_id_list)
        )

        transient_list, ra_list, dec_list, diff_id_list, warp_id_list, stack_id_list = (
            np.atleast_1d(np.asarray(transient_list)),
            np.atleast_1d(np.asarray(ra_list)),
            np.atleast_1d(np.asarray(dec_list)),
            np.atleast_1d(np.asarray(diff_id_list)),
            np.atleast_1d(np.asarray(warp_id_list)),
            np.atleast_1d(np.asarray(stack_id_list)),
        )
        ...
        count = 1
        for snid, ra, dec, diff_id in zip(
            transient_list, ra_list, dec_list, diff_id_list
        ):
            if diff_id is None:
                continue
        ...
        for snid, ra, dec, warp_id in zip(
            transient_list, ra_list, dec_list, warp_id_list
        ):
            if warp_id is None:
                continue
            skycell_str = skycelldict[snid]
        ...
        for snid, ra, dec, stack_id in zip(
            transient_list, ra_list, dec_list, stack_id_list
        ):
            if stack_id is None:
                continue
    ...
    def forcedphot_request(
        ...
        transient_list, ra_list, dec_list, mjd_list, filt_list, diff_id_list = (
            ...
            np.atleast_1d(np.asarray(ra_list)),
            np.atleast_1d(np.asarray(dec_list)),
            np.atleast_1d(np.asarray(mjd_list)),
            np.atleast_1d(np.asarray(filt_list)),
            np.atleast_1d(np.asarray(diff_id_list)),
        )
        ...
        for snid_unq in np.unique(transient_list):
            ...
            for snid, ra, dec, mjd, filt, diff_id in zip(
                transient_list[transient_list == snid_unq],
                ra_list[transient_list == snid_unq],
                dec_list[transient_list == snid_unq],
                mjd_list[transient_list == snid_unq],
                filt_list[transient_list == snid_unq],
                diff_id_list[transient_list == snid_unq],
            ):
                if diff_id is None or diff_id == "NULL":
                    continue
                data.add_row(
                    (
                        "forcedphot_ysebot_{}".format(count),
                        "gpc2",
                        ra,
                        dec,
                        ra,
    ...
    def submit_to_ipp(self, filename_or_obj):
        ...
        if type(filename_or_obj) == str:
            files = {"filename": open(filename, "rb")}
        else:
            files = {"filename": filename_or_obj.getvalue()}
