...
def get_gaia_list(look_back_days):
    ...
    datelist = np.array(
        [
            ...
            / 86400
            for date in gaia_list[" Date"].values
        ]
    )
...
def get_gaia_phot(name, targets):
    ...
    if sum(index) > 0:
        ...
        data = pd.read_csv(
            "http://gsaweb.ast.cam.ac.uk/alerts/alert/" + gaia_name + "/lightcurve.csv/"
        )
        return data
    else:
        return
...
class GaiaLC(CronJobBase):
    ...
    def do(self):
        ...
        print(
            "Gaia -> YSE_PZ took %.1f seconds for %i transients"
    ...
    def main(self):
        ...
        transients = AlternateTransientNames.objects.filter(
            name__startswith="Gaia"
        ).filter(
            transient__disc_date__gt=datetime.datetime.now()
        ...
        for i, t in enumerate(transients):
            ...
            for i, d in enumerate(data):
                if not isinstance(data[i], float):
                    if (
                        data[i] == "untrusted"
                        or data[i].startswith("average")
                        or data[i] == "nan"
                    ):
                        continue
                if float(data[i]) != float(data[i]):
                    continue

                flux = 10 ** (-0.4 * (float(data[i]) - 27.5))
                flux_err = 1.086 * 0.01 * flux
                phot_upload_dict = {
                    "obs_date": data.index[i][0].replace(" ", "T"),
                    "band": "G",
                    "groups": ["Public"],
                    "mag": float(data[i]),
                    "mag_err": 0.01,
