    ...
    def do(
        ...
        try:
            ...
            transients = Transient.objects.filter(
                Q(host__isnull=False) & Q(host__photo_z_internal__isnull=True)
            )
            ...
            myquery += (
                "g.dered_u / g.dered_z as u_over_z, g.petroR90_r/g.petroR50_r AS C "
            )
            ...
            def Filter():
                ...
                dense1 = keras.layers.Dense(20, activation=keras.activations.tanh)(
                    IN
                )  # filter has 20, tanh
                drop1 = keras.layers.Dropout(0.05)(dense1)

                dense2 = keras.layers.Dense(20, activation=keras.activations.tanh)(
                    drop1
                )  # filter has 20, tanh
                drop2 = keras.layers.Dropout(0.05)(dense2)  # 0.1

                dense3 = keras.layers.Dense(20, activation=keras.activations.tanh)(
                    drop2
                )  # filter has 20, tanh
                drop3 = keras.layers.Dropout(0.05)(dense3)

                dense4 = keras.layers.Dense(20, activation=keras.activations.tanh)(
                    drop3
                )  # filter has 20, tanh
                drop4 = keras.layers.Dropout(0.05)(dense4)  # 0.1

                dense5 = keras.layers.Dense(2, activation=keras.activations.softmax)(
                    drop4
                )  # filter has 2
            ...
            def MLP():
                IN = keras.layers.Input((17,))
                dense1 = keras.layers.Dense(45, activation=keras.activations.relu)(
                    IN
                )  # filter has 20, tanh
                drop1 = keras.layers.Dropout(0.05)(dense1)

                dense2 = keras.layers.Dense(45, activation=keras.activations.relu)(
                    drop1
                )  # filter has 20, tanh
                drop2 = keras.layers.Dropout(0.05)(dense2)  # 0.1

                dense3 = keras.layers.Dense(45, activation=keras.activations.relu)(
                    drop2
                )  # filter has 20, tanh
                drop3 = keras.layers.Dropout(0.05)(dense3)

                dense4 = keras.layers.Dense(45, activation=keras.activations.relu)(
                    drop3
                )  # filter has 20, tanh
                drop4 = keras.layers.Dropout(0.05)(dense4)  # 0.1

                dense5 = keras.layers.Dense(
                    NB_BINS, activation=keras.activations.softmax
                )(
                    drop4
                )  # filter has 2
            ...
            Filter_model.load_weights(
                "%s/%s" % (djangoSettings.STATIC_ROOT, filter_filepath)
            )
            ...
            for i in range(len(point_estimates)):
                error[i] = np.std(
                    np.random.choice(
                        a=range_z, size=1000, p=posterior[i, :], replace=True
                    )
                )  # this could be parallized i'm sure
