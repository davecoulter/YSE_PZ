...
class processTNS:
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        parser.add_argument(
            "--nedradius", default=5, type=float, help="NED search radius, in arcmin"
        )
        parser.add_argument(
            "--ndays",
            default=None,
        ...
        if config:
            ...
            parser.add_argument(
                "--tnsapi",
                default=config.get("main", "tnsapi"),
                type=str,
                help="TNS API URL (default=%default)",
            )
            parser.add_argument(
                "--tnsapikey",
                default=config.get("main", "tnsapikey"),
                type=str,
                help="TNS API key (default=%default)",
            )
            parser.add_argument(
                "--tns_bot_id",
                default=config.get("main", "tns_bot_id"),
                type=str,
                help="TNS API key (default=%default)",
            )
            parser.add_argument(
                "--tns_bot_name",
                default=config.get("main", "tns_bot_name"),
                type=str,
                help="TNS API key (default=%default)",
            )
            ...
            parser.add_argument(
                "--tns_recent_ndays",
                default=config.get("main", "tns_recent_ndays"),
                type=str,
                help="time interval for grabbing recent TNS events (default=%default)",
            )
            parser.add_argument(
                "--tns_fastupdates_nminutes",
                default=config.get("main", "tns_fastupdates_nminutes"),
                type=str,
            ...
            parser.add_argument(
                "--login", default="", type=str, help="gmail login (default=%default)"
            )
            parser.add_argument(
                "--password",
                default="",
            ...
            parser.add_argument(
                "--tnsapi", default="", type=str, help="TNS API URL (default=%default)"
            )
            parser.add_argument(
                "--tnsapikey",
                default="",
            ...
            parser.add_argument(
                "--ztfurl", default="", type=str, help="ZTF URL (default=%default)"
            )
            ...
            parser.add_argument(
                "--SMTP_HOST", default="", type=str, help="SMTP host (default=%default)"
            )
            parser.add_argument(
                "--SMTP_PORT", default="", type=str, help="SMTP port (default=%default)"
            )
    ...
    def getTNSData(self, jd, obj, sc, ebv):
        ...
        if jd:
            ...
            z = jd["redshift"]
            if z:
                TransientDict["redshift"] = float(z)
    ...
    def getZTFPhotometry(self, sc):
        ...
        if "results" in schema.keys():
            PhotUploadAll = {"mjdmatchmin": 0.01, "clobber": self.clobber}
            photometrydict = {
                "instrument": "ZTF-Cam",
                "obs_group": "ZTF",
                "photdata": {},
    ...
    def get_PS_DR2_data(self, sc):
        ...
        if results[1]:
            ...
            query += (
                "join Filter f on d.filterID=f.filterID order by d.filterID, obsTime"
            )
            ...
            for row in ascii.read(results):
                ...
                obstime = datetime.strftime(tobj.to_datetime(), "%Y-%m-%dT%H:%M:%S.%f")[
                    :-3
                ]
                photdata[obstime] = {}
    ...
    def getTNSPhotometry(self, jd, PhotUploadAll=None):
        ...
        for ins in np.unique(tinst):
            ...
            if np.array(obsgroups)[np.array(tinst) == ins][0] is None:
                ...
                obsgroup = re.sub(
                    r"[^\x00-\x7f]", r"", np.array(obsgroups)[np.array(tinst) == ins][0]
                )

            photometrydict = {"instrument": ins, "obs_group": obsgroup, "photdata": {}}

            for f, k in zip(np.unique(tfilt), range(len(np.unique(tfilt)))):
                # put in the photometry
                for m, me, od, df in zip(
                    ...
                    if not m and not me:
                        continue  # and not flx and not fe: continue
                    PhotUploadDict = {
        ...
        if nondetectdate:
            nondetectdate = nondetectdate.replace(" ", "T")
        return (
            PhotUploadAll,
            nondetectdate,
            nondetectmaglim,
    ...
    def getTNSSpectra(self, jd, sc):
        ...
        for s, si, so, sog in zip(specfiles, specinst, specobsdate, specobsgroup):
            ...
            try:
                dlfileresp = get_file(
                    s, self.tnsapikey, self.tns_bot_id, self.tns_bot_name
                )
            ...
            for line in fin:
                ...
                for key in keyslist:
                    if (
                        line.lower().startswith("# %s " % key)
                        and key not in Spectrum.keys()
                    ):
                        Spectrum[key] = (
            ...
            if ":" in Spectrum["ra"]:
                scspec = SkyCoord(
                    Spectrum["ra"], Spectrum["dec"], FK5, unit=(u.hourangle, u.deg)
                )
            ...
            try:
                spc = {}
                spc["wavelength"] = np.loadtxt(
                    "spec_tns_upload.txt",
                    unpack=True,
                    usecols=[np.where(header == "wavelength")[0][0]],
                )
                spc["flux"] = np.loadtxt(
                    "spec_tns_upload.txt",
                    unpack=True,
                    usecols=[np.where(header == "flux")[0][0]],
                )
            except:
                raise RuntimeError(
                    """Error : incorrect file format.
            ...
            if "fluxerr" in header:
                spc["fluxerr"] = np.loadtxt(
                    self.options.inputfile,
                    unpack=True,
                    usecols=[np.where(header == "fluxerr")[0][0]],
                )
            for key in keyslist:
            ...
            if "fluxerr" in spc.keys():
                for w, f, df in zip(spc["wavelength"], spc["flux"], spc["fluxerr"]):
                    if f == f:
                        SpecDict = {"wavelength": w, "flux": f, "flux_err": df}
                        SpecData[w] = SpecDict
            else:
                for w, f in zip(spc["wavelength"], spc["flux"]):
                    if f == f:
                        SpecDict = {"wavelength": w, "flux": f, "flux_err": None}
                        SpecData[w] = SpecDict
    ...
    def getNEDData(self, jd, sc, ned_table):
        ...
        if ned_table is not None:
            ...
            galaxy_candidates = np.asarray(
                [entry.decode("utf-8") for entry in ned_table["Type"]]
            )
        ...
        if "name" not in hostdict.keys():
            for z, name, ra, dec, sep, mag in zip(
                noz_galaxy_zs,
                noz_galaxy_names,
                noz_galaxy_ras,
                noz_galaxy_decs,
    ...
    def UpdateFromTNS(
        ...
        transienturl = "%stransients?limit=1000&format=json&%s&offset=%i" % (
            self.dburl,
            argstring,
            offsetcount,
        )
        ...
        while len(schema["results"]) == 1000:
            ...
            for transient in schema["results"]:
                ...
                decs.append(transient["dec"])
        if self.redohost:
            nsn = self.GetAndUploadAllData(
                objs, ras, decs, doGHOST=True, doEBV=True, doTNS=doTNS
            )
        else:
            nsn = self.GetAndUploadAllData(
                objs, ras, decs, doGHOST=False, doEBV=False, doTNS=doTNS
            )
        return nsn
    ...
    def GetRecentEvents(self, ndays=None, doTNS=True):
        # date_format = '%Y-%m-%d'
        datemin = (
            datetime.now() - timedelta(days=ndays)
        ).isoformat()  # strftime(date_format)
        search_obj = [
            ("ra", ""),
            ("dec", ""),
            ("radius", ""),
            ...
            ("objname", ""),
            ("internal_name", ""),
            ("public_timestamp", datemin),
        ]

        response = search(
            self.tnsapi, search_obj, self.tnsapikey, self.tns_bot_id, self.tns_bot_name
        )
        ...
        if len(objs):
            nsn = self.GetAndUploadAllData(
                objs, ras, decs, doGHOST=self.redohost, doEBV=self.redohost, doTNS=doTNS
            )
        else:
            nsn = 0
        return nsn
    ...
    def GetRecentMissingEvents(self, ndays=None, doTNS=True):
        ...
        objs, ras, decs = [], [], []
        for jd in json_data["data"]["reply"]:
            if len(Transient.objects.filter(name=jd["objname"])):
                continue
            TNSGetSingle = [
                ("objname", jd["objname"]),
                ("photometry", "1"),
    ...
    def ProcessTNSEmails(self):
        ...
        for i in range(len(msg_ids)):
            ...
            for obj in re.findall(reg_obj, body):
                objs.append(obj)
            for ra in re.findall(reg_ra, body):
                ras.append(ra)
            for dec in re.findall(reg_dec, body):
                decs.append(dec)
            if len(objs) != len(ras):
                import pdb
    ...
    def GetAndUploadAllData(
        self, objs, ras, decs, doGHOST=True, doEBV=True, doTNS=True
    ):  # doNED=True
        TransientUploadDict = {}
        ...
        if doGHOST:
            ...
            try:
                ...
                if type(ras[0]) == float:
                    ...
                    scall = [
                        SkyCoord(r, d, unit=(u.hourangle, u.deg))
                        for r, d in zip(ras, decs)
                    ]

                ghost_hosts = getTransientHosts(
                    objs, scall, verbose=True, starcut="gentle", ascentMatch=False
                )
                ghost_hosts = calc_photoz(ghost_hosts)
                os.system(
                    f"rm -r transients_{datetime.utcnow().isoformat().split('T')[0].replace('-','')}*"
                )
            except:
                print("GHOST timeout!")
                ned_timeout = True
        ...
        for j, jd in zip(range(len(objs)), json_data):
            ...
            try:
                if jd:
                    photdict, nondetectdate, nondetectmaglim, nondetectfilt, nondetectins = self.getTNSPhotometry(
                        jd, PhotUploadAll=photdict
                    )
...
def get_file(url, api_key, tns_bot_id, tns_bot_name):
    try:
        ...
        response = requests.post(
            url, headers=headers, data={"api_key": api_key}, stream=True
        )
        return response
    except Exception as e:
        print("Error message : \n" + str(e))
...
class TNS_emails(CronJobBase):
    ...
    def do(self):
        ...
        print(
            "TNS -> YSE_PZ took %.1f seconds for %i transients"
...
class TNS_Ignore_updates(CronJobBase):
    ...
    def do(self):
        ...
        # execute only if run as a script
        print(
            "running TNS updates for Ignore transients at {}".format(
        ...
        try:
            tnsproc.noupdatestatus = True
            nsn = tnsproc.UpdateFromTNS(
                ndays=30, allowed_statuses=["Ignore"], doTNS=False
            )
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            nsn = 0
...
class TNS_recent_realtime(CronJobBase):
    ...
    def do(self):
        ...
        try:
            tnsproc.noupdatestatus = True
            nsn = tnsproc.GetRecentEvents(
                ndays=tnsproc.tns_fastupdates_nminutes / 60.0 / 24.0
            )
        except Exception as e:
            exc_type, exc_obj, exc_tb = sys.exc_info()
            nsn = 0
