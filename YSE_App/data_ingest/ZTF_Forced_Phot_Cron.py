...
class ForcedPhot(CronJobBase):
    ...
    def add_options(self, parser=None, usage=None, config=None):
        ...
        if config:
            parser.add_argument(
                "--ztfforcedphotpass",
                default=config.get("yse_forcedphot", "ztfforcedphotpass"),
                type=str,
                help="ZTF password (default=%default)",
            )
            parser.add_argument(
                "--ztfforcedtmpdir",
                default=config.get("yse_forcedphot", "ztfforcedtmpdir"),
                type=str,
    ...
    def main(self):
        ...
        for l in logs:
            if l.transient is None:
                continue
            if l.transient.name in list_already_done:
                continue
            ...
            output_files = ztf.get_ztf_fp(
                log_file_name,
                directory_path=f"{self.options.ztfforcedtmpdir}/forced_phot_out",  # directory_path='/data/yse_pz/tmp/forced_phot_out',
                source_name=l.transient.name,
                verbose=True,
            )
            # except Exception as e:
            #    print(e)
            ...
            try:
                # /data/yse_pz
                data = at.Table.read(
                    f"{self.options.ztfforcedtmpdir}/forced_phot_out/%s/%s_lc.txt"
                    % (l.transient.name, l.transient.name),
                    format="ascii",
                    header_start=0,
                )
            except:
            ...
            DataForYSEPZ[l.transient.name] = {
                "name": l.transient.name,
                "ra": l.transient.ra,
                "dec": l.transient.dec,
                "obs_group": "Unknown",
                "status": "Ignore",
            ...
            PhotUploadAll["ZTF"] = {"instrument": "ZTF-Cam", "obs_group": "ZTF"}
            PhotUploadAll["ZTF"]["photdata"] = {}

            # loop over the photometry
            for i, d in enumerate(data):
                if d["forcediffimfluxap,"] == "null":
                    continue

                mag = -2.5 * np.log10(float(d["forcediffimfluxap,"])) + float(
                    d["zpdiff,"]
                )
                mag_err = (
                    1.086
                    * float(d["forcediffimfluxuncap,"])
                    / float(d["forcediffimfluxap,"])
                )
                if mag != mag:
                    mag = None
                    mag_err = None
                ...
                PhotDataDict = {
                    "obs_date": jd_to_date(float(d["jd,"])),
                    "mag": mag,
                    "mag_err": mag_err,
                    "band": "%s-ZTF" % d["filter,"][-1],
                    "data_quality": 0,
                    "diffim": 1,
                    "flux": float(d["forcediffimfluxap,"])
                    * 10 ** (-0.4 * (float(d["zpdiff,"]) - 27.5)),
                    "flux_err": float(d["forcediffimfluxuncap,"])
                    * 10 ** (-0.4 * (float(d["zpdiff,"]) - 27.5)),
