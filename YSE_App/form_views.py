...
from .common.utilities import coordstr_to_decimal
...
class AddTransientFollowupFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            if instance.transient.status.name in [
                "New",
                "Watch",
                "Ignore",
                "Interesting",
            ...
            if form.cleaned_data["comment"]:
                log = Log(
                    transient_followup=TransientFollowup.objects.get(id=instance.id),
            ...
            data = {"data": data_dict, "message": "Successfully submitted form data."}
...
class AddClassicalResourceFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            instance.end_date_valid = form.cleaned_data[
                "observing_date"
            ...
            obsdatedict = {
                ...
                "resource": instance,
                "night_type": ClassicalNightType.objects.filter(name="Full")[0],
...
class AddTransientObservationTaskFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            data_dict[
                "observatory"
            ] = instance.instrument_config.instrument.telescope.observatory.name
            data_dict[
                "telescope"
            ] = instance.instrument_config.instrument.telescope.name
            data_dict["instrument"] = instance.instrument_config.instrument.name
            data_dict["modified_by"] = instance.modified_by.username

            data = {"message": "Successfully submitted form data.", "data": data_dict}
            return JsonResponse(data)
        else:
            return response
...
class AddSurveyFieldFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            # danger!
            obs_requests = SurveyObservation.objects.filter(
                survey_field__field_id=instance.field_id
            ).filter(mjd_requested__range=(instance.first_mjd, instance.last_mjd))
            obs_requests.delete()
            ...
            for i, m in enumerate(mjd):
                ...
                band1 = PhotometricBand.objects.filter(
                    name=band1name, instrument__name=instance.instrument.name
                )[0]
                band2 = PhotometricBand.objects.filter(
                    name=band2name, instrument__name=instance.instrument.name
                )[0]
...
class AddSurveyObsFormView(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not is_ajax(self.request):
            auth_method, credentials = self.request.META["HTTP_AUTHORIZATION"].split(
                " ", 1
            )
                ...
                for s in sb.survey_fields.all():
                    ...
                    if len(previous_msb):
                        ...
                        previous_obs = (
                            SurveyObservation.objects.filter(
                                survey_field=previous_field
                            )
                            .filter(Q(obs_mjd__lt=m) | Q(mjd_requested__lt=m))
                            .order_by("-obs_mjd")
                            .order_by("-mjd_requested")
                            .select_related()
                        )
                    else:
                        previous_obs = (
                            SurveyObservation.objects.filter(survey_field=s)
                            .filter(Q(obs_mjd__lt=m) | Q(mjd_requested__lt=m))
                            .order_by("-obs_mjd")
                            .order_by("-mjd_requested")
                            .select_related()
                    ...
                    def previous_obs_func(illum_min, illum_max):
                        ...
                        for p in previous_obs:
                            if p.obs_mjd:
                                mjd_prev = p.obs_mjd
                            elif p.mjd_requested:
                                mjd_prev = p.mjd_requested
                    ...
                    band1 = PhotometricBand.objects.filter(
                        name=band1name, instrument__name=s.instrument.name
                    )[0]
                    band2 = PhotometricBand.objects.filter(
                        name=band2name, instrument__name=s.instrument.name
                    )[0]
...
class AddOncallUserFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            for i, m in enumerate(mjd):
                t = Time(m, format="mjd")
                yse_date = YSEOnCallDate.objects.filter(
...
class AddTransientCommentFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            data_dict["modified_date"] = (
                instance.modified_date.strftime("%b. %-d, %Y, %H:%M ")
...
class AddFollowupNoticeFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            instance.modified_by = self.request.user
            try:
                instance.profile = Profile.objects.filter(user=self.request.user)[0]
            except:
                data = {
                    "message": """User %s has no profile object in the YSE_PZ database.
Contact D. Jones or D. Coulter."""
                    % self.request.user
                }
                return JsonResponse(data)
    ...
    def form_valid(self, form):
        ...
        if is_ajax(self.request):
            ...
            if "goodman" in form.cleaned_data["instrument"].name.lower():
                resource = ClassicalResource.objects.filter(
                    telescope__name=form.cleaned_data["instrument"].telescope
                ).filter(principal_investigator__name="Dimitriadis")
                if not len(self.request.user.groups.all().filter(name="SOAR")):
                    data_dict = {
                        "errors": "you need to be in the SOAR permissions group to submit this request!",
                        "errorflag": 1,
                    }
                    data = {
                        "data": data_dict,
                ...
                resource = ToOResource.objects.filter(
                    telescope__name=form.cleaned_data["instrument"].telescope
                )  # .\
                # filter(principal_investigator__name='Kilpatrick')
                if not len(self.request.user.groups.all().filter(name="LCOGT")):
                    data_dict = {
                        "errors": "you need to be in the LCOGT permissions group to submit this request!",
                        "errorflag": 1,
                    }
                    data = {
                        "data": data_dict,
            ...
            resource = resource.filter(
                Q(
                    begin_date_valid__lt=form.cleaned_data["spectrum_valid_start"]
                    + datetime.timedelta(1)
                )
                & Q(
                    end_date_valid__gt=form.cleaned_data["spectrum_valid_stop"]
                    - datetime.timedelta(1)
                )
            )
            ...
            if not len(resource):
                data_dict = {
                    "errors": "could not find a matching resource, make sure the dates are valid and the program is still active!",
                    "errorflag": 1,
                }
                data = {
                    "data": data_dict,
            ...
            lcogt.main(
                ...
                tf.transient.ra,
                tf.transient.dec,
                form.cleaned_data["exp_time"],
                form.cleaned_data["instrument"].telescope.name.split()[0],
                form.cleaned_data["spectrum_valid_start"]
                .replace(tzinfo=None)
                .isoformat(),
                form.cleaned_data["spectrum_valid_stop"]
                .replace(tzinfo=None)
                .isoformat(),
            )

            data_dict = {"errors": "", "errorflag": 0}
            data = {"data": data_dict, "message": "Successfully submitted form data."}
            return JsonResponse(data)
        else:
            return response
