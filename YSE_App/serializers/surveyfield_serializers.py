...
class SurveyFieldSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        instance.ztf_field_id = validated_data.get(
            "ztf_field_id", instance.ztf_field_id
        )
...
class SurveyObservationSerializer(serializers.HyperlinkedModelSerializer):
    survey_field = serializers.HyperlinkedRelatedField(
        queryset=SurveyField.objects.all(), view_name="surveyfield-detail"
    )
    status = serializers.HyperlinkedRelatedField(
        queryset=TaskStatus.objects.all(), view_name="taskstatus-detail"
    )
    photometric_band = serializers.HyperlinkedRelatedField(
    ...
    def update(self, instance, validated_data):
        instance.mjd_requested = validated_data.get(
            "mjd_requested", instance.mjd_requested
        )
        instance.obs_mjd = validated_data.get("obs_mjd", instance.obs_mjd)
        instance.survey_field = validated_data.get(
            "survey_field", instance.survey_field
        )
        instance.status = validated_data.get("status", instance.status)
        instance.exposure_time = validated_data.get(
            "exposure_time", instance.exposure_time
        )
        instance.photometric_band = validated_data.get(
            "photometric_band", instance.photometric_band
        )

        instance.status = validated_data.get("status", instance.status)
        instance.pos_angle_deg = validated_data.get(
            "pos_angle_deg", instance.pos_angle_deg
        )

        instance.fwhm_major = validated_data.get("fwhm", instance.fwhm_major)
        instance.eccentricity = validated_data.get(
        ...
        instance.n_good_skycell = validated_data.get(
            "n_good_skycell", instance.n_good_skycell
        )
