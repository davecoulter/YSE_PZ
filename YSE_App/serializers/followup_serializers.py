...
class TransientFollowupSerializer(serializers.HyperlinkedModelSerializer):
    ...
    status = serializers.HyperlinkedRelatedField(
        queryset=FollowupStatus.objects.all(), view_name="followupstatus-detail"
    )

    too_resource = serializers.HyperlinkedRelatedField(
        queryset=ToOResource.objects.all(),
        allow_null=True,
        required=False,
        view_name="tooresource-detail",
    )
    classical_resource = serializers.HyperlinkedRelatedField(
        queryset=ClassicalResource.objects.all(),
        allow_null=True,
        required=False,
        view_name="classicalresource-detail",
    )  # , lookup_field="id")
    queued_resource = serializers.HyperlinkedRelatedField(
        queryset=QueuedResource.objects.all(),
        allow_null=True,
        required=False,
        view_name="queuedresource-detail",
    )
    ...
    def update(self, instance, validated_data):
        instance.transient_id = validated_data.get("transient", instance.transient)
        instance.status_id = validated_data.get("status", instance.status)
        instance.too_resource_id = validated_data.get(
            "too_resource", instance.too_resource
        )
        instance.classical_resource_id = validated_data.get(
            "classical_resource", instance.classical_resource
        )
        instance.queued_resource_id = validated_data.get(
            "queued_resource", instance.queued_resource
        )
        ...
        instance.spec_priority = validated_data.get(
            "spec_priority", instance.spec_priority
        )
        instance.phot_priority = validated_data.get(
            "phot_priority", instance.phot_priority
        )
        instance.offset_star_ra = validated_data.get(
            "offset_star_ra", instance.offset_star_ra
        )
        instance.offset_star_dec = validated_data.get(
            "offset_star_dec", instance.offset_star_dec
        )
        instance.offset_north = validated_data.get(
