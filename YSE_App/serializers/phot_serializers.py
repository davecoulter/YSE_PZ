        ...
        if groups_exist:
            ...
            if not validGroups:
                raise PermissionDenied(
                    {
                        "message": "You don't have permission to create transient photometry with",
                        "group": groups,
                    }
                )
    ...
    def create(self, validated_data):
        ...
        if groups_exist:
            ...
            if not validGroups:
                raise PermissionDenied(
                    {
                        "message": "You don't have permission to create host photometry with",
                        "group": groups,
                    }
                )
...
class TransientPhotDataSerializer(serializers.HyperlinkedModelSerializer):
    photometry = serializers.HyperlinkedRelatedField(
    ...
    def create(self, validated_data):
        ...
        parent_photometry = TransientPhotometry.objects.get(
            pk=transient_phot_data.photometry.id
        )
        user_groups = self.context["request"].user.groups.all()
    ...
    def update(self, instance, validated_data):
        ...
        instance.flux_zero_point = validated_data.get(
            "flux_zero_point", instance.flux_zero_point
        )
        ...
        instance.discovery_point = validated_data.get(
            "discovery_point", instance.discovery_point
        )

        new_parent_photometry = TransientPhotometry.objects.get(
            pk=instance.photometry_id.id
        )
        user_groups = self.context["request"].user.groups.all()
...
class TransientImageSerializer(serializers.HyperlinkedModelSerializer):
    phot_data = serializers.HyperlinkedRelatedField(
        queryset=TransientPhotData.objects.all(), view_name="transientphotdata-detail"
    )
    ...
    def update(self, instance, validated_data):
        ...
        instance.zero_point_err = validated_data.get(
            "zero_point_err", instance.zero_point_err
        )
...
class TransientDiffImageSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def update(self, instance, validated_data):
        instance.phot_data_id = validated_data.get("phot_data", instance.phot_data)
        instance.valid_pixels = validated_data.get(
            "valid_pixels", instance.valid_pixels
        )

        instance.postage_stamp_file = validated_data.get(
            "postage_stamp_file", instance.postage_stamp_file
        )
        instance.postage_stamp_ref = validated_data.get(
            "postage_stamp_ref", instance.postage_stamp_ref
        )
        instance.postage_stamp_diff = validated_data.get(
            "postage_stamp_diff", instance.postage_stamp_diff
        )
        instance.postage_stamp_file_fits = validated_data.get(
            "postage_stamp_file_fits", instance.postage_stamp_file_fits
        )
        instance.postage_stamp_ref_fits = validated_data.get(
            "postage_stamp_ref_fits", instance.postage_stamp_ref_fits
        )
        instance.postage_stamp_diff_fits = validated_data.get(
            "postage_stamp_diff_fits", instance.postage_stamp_diff_fits
        )
...
class HostImageSerializer(serializers.HyperlinkedModelSerializer):
    phot_data = serializers.HyperlinkedRelatedField(
        queryset=HostPhotData.objects.all(), view_name="hostphotdata-detail"
    )
