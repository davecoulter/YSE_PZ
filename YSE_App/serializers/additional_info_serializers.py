...
class TransientWebResourceSerializer(serializers.HyperlinkedModelSerializer):
    ...
    information_source = serializers.HyperlinkedRelatedField(
        queryset=InformationSource.objects.all(), view_name="informationsource-detail"
    )
    ...
    def update(self, instance, validated_data):
        ...
        instance.information_source_id = validated_data.get(
            "information_source", instance.information_source
        )

        instance.information_text = validated_data.get(
            "information_text", instance.information_text
        )
        instance.resource_url = validated_data.get(
...
class HostWebResourceSerializer(serializers.HyperlinkedModelSerializer):
    host = serializers.HyperlinkedRelatedField(
        queryset=Host.objects.all(), lookup_field="id", view_name="host-detail"
    )
    information_source = serializers.HyperlinkedRelatedField(
        queryset=InformationSource.objects.all(), view_name="informationsource-detail"
    )
