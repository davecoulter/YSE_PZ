...
class TransientFollowupForm(ModelForm):
    ...
    qs = (
        ClassicalResource.objects.filter(
            end_date_valid__gt=timezone.now() - timedelta(days=1)
        )
...
class ClassicalResourceForm(ModelForm):
    ...
    class Meta:
        ...
        fields = ["telescope", "principal_investigator"]
...
class SurveyFieldForm(ModelForm):
    ...
    if len(qs):
        instrument = forms.ModelChoiceField(queryset=qs, initial=qs[0], required=False)

    class Meta:
        model = SurveyField
        fields = ["field_id", "cadence", "ztf_field_id", "instrument"]
...
class SurveyObsForm(ModelForm):
    ...
    qs = [
        (i["ztf_field_id"], i["ztf_field_id"])
        for i in SurveyField.objects.filter(~Q(obs_group__name="ZTF"))
        .values("ztf_field_id")
        .distinct()
        .order_by("ztf_field_id")
    ]

    if len(qs):
        ztf_field_id = forms.MultipleChoiceField(
            choices=qs, initial=qs[0], required=True
        )
    else:
        ztf_field_id = forms.MultipleChoiceField(choices=[], required=True)
...
class OncallForm(ModelForm):
    ...
    qs = (
        User.objects.all()
        .filter(groups__name="YSE")
        .filter(~Q(username="admin"))
        .order_by("username")
    )
    if len(qs):
        user = forms.ModelChoiceField(queryset=qs, initial=qs[0], required=False)
...
class AddDashboardQueryForm(ModelForm):
    ...
    query_names = [("", "---------")] + [
        (q, q) for i, q in enumerate(python_query_reg.all.keys())
    ]
...
class SpectrumUploadForm(ModelForm):
    ...
    qs = [
        (i["name"], i["name"])
        for i in Group.objects.all().values("name").distinct().order_by("name")
    ]
    permissions = forms.MultipleChoiceField(choices=qs, required=False)
    ...
    instrument = forms.ModelChoiceField(
        Instrument.objects.filter(Q(name__in=spec_instruments))
    )
    # import pdb; pdb.set_trace()
    class Meta:
        model = TransientSpectrum
        fields = ("transient", "ra", "dec", "spec_phase")  # ,'obs_group','instrument')
