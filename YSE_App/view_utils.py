...
def get_recent_phot_for_host(user, host_id=None):
    allowed_phot = PhotometryService.GetAuthorizedHostPhotometry_ByUser_ByHost(
        user, host_id
    )
...
def get_obs_nights_happening_soon(user):
    allowed_nights = ObservingResourceService.GetAuthorizedClassicalObservingDate_ByUser(
        user
    ).select_related()
...
def get_too_resources(user):
    allowed_too_resource = ObservingResourceService.GetAuthorizedToOResource_ByUser(
        user
    )
...
def getTimeUntilRiseSet(ra, dec, date, lat, lon, elev, utc_off):
    ...
    target_rise_time = tel.target_rise_time(
        time, sc, horizon=18 * u.deg, which="previous"
    )
    target_set_time = tel.target_set_time(
        time, sc, horizon=18 * u.deg, which="previous"
    )
...
class finder(TemplateView):
    ...
    def finderchart(self, request, transient_id, clobber=False):
        ...
        fig = Figure()
        if (
            not os.path.exists(options.outputFinderFileName)
            or not os.path.exists(options.outputOffsetFileName)
            or clobber
        ):
            ax = fig.add_axes([0.2, 0.3, 0.6, 0.6])
            canvas = FigureCanvas(fig)
            ax, offdictlist = find.mkChart(
                options.ra,
                options.dec,
            ...
            offid, ra_str, dec_str, ra_off, dec_off, mag = np.loadtxt(
                outputOffsetFileName, dtype="str", unpack=True
            )
            offdictlist = []
        ...
        context = {"t": transient, "offsets": offdictlist}
...
def airmassplot(request, transient_id, obs_id, telescope_id):
    ...
    observe_time = night_start + datetime.timedelta(
        hours=6
    )  # + datetime.timedelta(night_end-night_end #delta_t*np.linspace(0, 1, 75)
    plot_airmass(target, tel, observe_time, ax=ax)

    yr, mn, day, hr, minu, sec = (
        night_start.iso.replace(":", " ").replace("-", " ").split()
    )
    ...
    yr, mn, day, hr, minu, sec = (
        night_end.iso.replace(":", " ").replace("-", " ").split()
    )
...
def view_yse_fields(request):
    ...
    survey_obs = (
        SurveyObservation.objects.filter(
            Q(mjd_requested__gte=date_to_mjd(obs_date_now))
            | Q(obs_mjd__gte=date_to_mjd(obs_date_now))
        )
        .filter(
            Q(mjd_requested__lte=date_to_mjd(obs_date_now) + 1)
            | Q(obs_mjd__lte=date_to_mjd(obs_date_now) + 1)
        )
    ...
    survey_obs = (
        SurveyObservation.objects.filter(
            Q(mjd_requested__gte=date_to_mjd(obs_date_last) - 1)
            | Q(obs_mjd__gte=date_to_mjd(obs_date_last) - 1)
        )
        .filter(
            Q(mjd_requested__lte=date_to_mjd(obs_date_last))
            | Q(obs_mjd__lte=date_to_mjd(obs_date_last))
        )
        .filter(survey_field__instrument__name__startswith="GPC")
        .select_related()
    )
    ...
    survey_obs = (
        SurveyObservation.objects.filter(
            Q(mjd_requested__gte=date_to_mjd(obs_date_last) - 2)
            | Q(obs_mjd__gte=date_to_mjd(obs_date_last) - 2)
        )
        .filter(
            Q(mjd_requested__lte=date_to_mjd(obs_date_last) - 1)
            | Q(obs_mjd__lte=date_to_mjd(obs_date_last) - 1)
        )
        .filter(survey_field__instrument__name__startswith="GPC")
        .select_related()
    )
    field_pk = survey_obs.values("survey_field").distinct()
    survey_fields_two_nights_ago = (
        SurveyField.objects.filter(pk__in=field_pk)
        .filter(~Q(obs_group__name="ZTF"))
        .select_related()
    )
    ...
    def map_survey_fields(f, metric_value=1.0):
        ...
        width_corr = 1.65 / np.abs(np.cos(f.dec_cen * np.pi / 180.0))
        ...
        for c in corners:
            ...
            if galcoord.l.degree > 180.0:
                theta2 = galcoord.l.degree - 360.0
            else:
...
def lightcurveplot_summary(request, transient_id, salt2=False):
    ...
    for p in photdata:
        ...
        if p.flux and np.abs(p.flux) > 1e10:
            continue
        # if p.data_quality:
        #   continue

        if (p.flux and p.mag and p.flux_err and p.flux / p.flux_err > 3) or (
            not p.flux and p.mag
        ):
            ...
            if p.mag_sys is None:
                magsys = np.append(mag, ["None"])
            else:
                magsys = np.append(mag, [p.mag_sys])
            if p.data_quality is None:
                data_quality = np.append(data_quality, ["Good"])
            else:
                data_quality = np.append(data_quality, [p.data_quality.name])
            if p.mag_err:
                magerr = np.append(magerr, p.mag_err)
            else:
                magerr = np.append(magerr, 0)
            ...
            if p.band.disp_symbol in py2bokeh_symboldict.keys():
                bandsym = np.append(
                    bandsym, str(py2bokeh_symboldict[p.band.disp_symbol])
                )
            else:
            ...
            if salt2:
                if str(p.band) in bandpassdict.keys():
                    ...
                    fluxerr = np.append(
                        fluxerr, flux_single * mag_err * 0.4 * np.log(10)
                    )
                    salt2band = np.append(salt2band, bandpassdict[str(p.band)])
                    if "bessell" in bandpassdict[str(p.band)]:
            ...
            if p.data_quality is None:
                ...
                upperlimdataquality = np.append(
                    upperlimdataquality, [p.data_quality.name]
                )
            if p.mag_sys is None:
                upperlimdataquality = np.append(upperlimmagsys, ["None"])
            else:
                upperlimmagsys = np.append(upperlimmagsys, [p.mag_sys])
    ...
    if transient.non_detect_limit and transient.non_detect_band:
        ...
        upperlimdate = np.append(
            upperlimdate, [transient.non_detect_date.strftime("%m/%d/%Y")]
        )
        ...
        upperlimbandcolor = np.append(
            upperlimbandcolor, transient.non_detect_band.disp_color
        )
        upperlimdataquality = np.append(upperlimdataquality, "Good")
    ...
    legend_it = []
    for bs, b, bc, bsym in zip(
        bandunq, allband[idx], allbandcolor[idx], allbandsym[idx]
    ):
        if bc != "None" and bc:
            color = bc
        else:
        ...
        try:
            p = plotmethod(
                mjd[bandstr == bs].tolist(),
                mag[bandstr == bs].tolist(),
                color=color,
                size=7,
                muted_alpha=0.2,
            )  # ,legend='%s - %s'%(
        except:
            raise RuntimeError(bs, b, bsym)
        ...
        if len(upperlimbandstr) and len(upperlimmjd[upperlimbandstr == bs]):
            p = ax.inverted_triangle(
                upperlimmjd[upperlimbandstr == bs].tolist(),
                upperlimmag[upperlimbandstr == bs].tolist(),
                color=color,
                size=7,
                muted_alpha=0.2,
            )
        ...
        for x, y, yerr in zip(
            mjd[bandstr == bs].tolist(),
            mag[bandstr == bs].tolist(),
            magerr[bandstr == bs].tolist(),
        ):
            err_xs.append((x, x))
            err_ys.append((y - yerr, y + yerr))
        ax.multi_line(
            err_xs, err_ys, color=color, muted_alpha=0.2
        )  # , legend='%s - %s'%(
        legend_it.append(("%s - %s" % (b.instrument.telescope.name, b.name), [p]))
        # b.instrument.telescope.name,b.name))
        # legend_it.append(('%s - %s'%(b.instrument.telescope.name,b.name), [p]))
    ...
    p = ax.line(
        today, 20, line_width=3, line_color="black"
    )  # ,legend='today (%i)'%today)
    legend_it.append(("today (%i)" % today, [p]))
    vline = Span(location=today, dimension="height", line_color="black", line_width=3)
    ax.add_layout(vline)
    ...
    if limmjd:
        ...
        ax.y_range = Range1d(
            np.max(np.append(mag[mjd > limmjd], upperlimmag[upperlimmjd > limmjd]))
            + 0.25,
        ...
        ax.y_range = Range1d(
            np.max(np.append(mag, upperlimmag)) + 0.25, np.min(mag) - 0.5
        )
        ...
        for m, t in zip(mjdrange, times):
            ...
            if tm_list[2] == "01":
                ...
                overridedict[m] = "%s %i, %i" % (
                    calendar.month_name[int(tm_list[1])][:3],
                    int(tm_list[2]),
                    int(tm_list[0]),
                )
    else:
    ...
    if salt2:
        ...
        data = Table(
            [salt2mjd, salt2band, flux, fluxerr, zp, zpsys],
        ...
        count = 0
        plotmjd = np.arange(
            result["parameters"][1] - 20, result["parameters"][1] + 50, 0.5
        )
        ...
        for bs, b, bc in zip(bandunq, band[idx], bandcolor[idx]):
            ...
            if bs in bandpassdict.keys() and bandpassdict[bs] in salt2band:
                salt2flux = fitted_model.bandflux(
                    ...
                    plotmjd,
                    zp=27.5,
                    zpsys=zpsys[bandpassdict[bs] == salt2band][0],
                )
...
def lightcurveplot_detail(request, transient_id, salt2=False):
    ...
    obs_dates_str = np.array(
        [p.strftime("%m/%d/%Y") for p in photdata.values_list("obs_date", flat=True)]
    )
    mjds = date_to_mjd(obs_dates)
    data_quality = np.array(
        [
            "Good" if p is None else p
            for p in photdata.values_list("data_quality__name", flat=True)
        ]
    )
    mag_sys = np.array(
        [
            "None" if p is None else p
            for p in photdata.values_list("mag_sys__name", flat=True)
        ]
    )
    ...
    instrument_name = np.array(
        [PhotometricBand.objects.get(pk=b).instrument.name for b in band]
    )
    ...
    disp_symbol = np.array(
        [PhotometricBand.objects.get(pk=b).disp_symbol for b in band]
    )
    ...
    ax = figure(
        plot_width=240, plot_height=240, sizing_mode="scale_width"
    )  # ,tooltips=TOOLTIPS)#'stretch_both')
    ...
    for bs, bn, b, bc, bsym, inn in zip(
        ...
        if salt2:
            ...
            if bandkey in bandpassdict.keys():
                ...
                salt2fluxerr = np.append(
                    salt2fluxerr, fluxes_single * salt2mag_errs * 0.4 * np.log(10)
                )
                salt2band = np.append(
                    salt2band, [bandpassdict[bandkey]] * len(salt2mag_errs)
                )
                if "bessell" in bandpassdict[bandkey]:
                    zpsys = np.append(zpsys, ["Vega"] * len(mag_errs[iBand]))
                else:
                    zpsys = np.append(zpsys, ["AB"] * len(mag_errs[iBand]))
        ...
        iPlot = (
            ...
            & (mags != None)
            & (mag_errs != None)
            & ((mag_errs_tmp <= 0.36) | (fluxes == None) | (flux_errs == None))
        )
        iPlotUlimFlux = (
            (band_name == bn)
            & (instrument_name == inn)
        ...
        mags_ulim = (
            ...
            * np.log10(
                (
                    fluxes[iPlotUlimFlux][iPlotUlimFlux2]
                    + 3 * flux_errs[iPlotUlimFlux][iPlotUlimFlux2]
                ).astype(float)
            )
            + flux_zpts[iPlotUlimFlux][iPlotUlimFlux2]
        )
        iPlotUlimFlux3 = mags_ulim == mags_ulim
        upperlimmag = np.append(upperlimmag, mags_ulim[iPlotUlimFlux3])
        upperlimmjd = np.append(
            upperlimmjd, mjds[iPlotUlimFlux][iPlotUlimFlux2][iPlotUlimFlux3].tolist()
        )
        ...
        p = plotmethod("x", "y", source=source, color=color, size=size, muted_alpha=0.2)
        g1_hover = HoverTool(renderers=[p], tooltips=TOOLTIPS, toggleable=False)
        ax.add_tools(g1_hover)

        source = ColumnDataSource(
            data=dict(
                x=mjds[iPlotUlimFlux][iPlotUlimFlux2].tolist(),
                ...
                band=["%s - %s" % (inn, bn)]
                * len(mjds[iPlotUlimFlux][iPlotUlimFlux2].tolist()),
            )
        )

        p = ax.inverted_triangle(
            "x", "y", source=source, color=color, size=5, muted_alpha=0.2
        )
        g1_hover = HoverTool(renderers=[p], tooltips=TOOLTIPS, toggleable=False)
        ax.add_tools(g1_hover)
        err_xs, err_ys = [], []
        for x, y, yerr in zip(
            mjds[iPlot].tolist(), mags[iPlot].tolist(), mag_errs[iPlot].tolist()
        ):
            err_xs.append((x, x))
            err_ys.append((y - yerr, y + yerr))
        ax.multi_line(
    ...
    if len(mjds[disc_points == 1]):
        ...
        ax.y_range = Range1d(
            np.max(
                np.append(
                    mags[mags != None][mjds[mags != None] > limmjd],
                    upperlimmag[upperlimmjd > limmjd],
                )
            )
            + 0.25,
        ...
        ax.y_range = Range1d(
            np.max(np.append(mags[mags != None], upperlimmag)) + 0.25,
            np.min(mags[mags != None]) - 0.5,
        )
    ...
    if z is not None:
        ...
        ax.add_layout(
            LinearAxis(y_range_name="Abs. Mag", axis_label="Abs. Mag"), "right"
        )
    ...
    if salt2:
        ...
        data = Table(
            [
                salt2mjd,
                salt2band,
                salt2flux.astype(float),
                salt2fluxerr.astype(float),
                zp,
                zpsys,
            ],
            names=["mjd", "band", "flux", "fluxerr", "zp", "zpsys"],
        ...
        pkguess = np.atleast_1d(
            salt2mjd[salt2flux / salt2fluxerr > 3][
                salt2flux[salt2flux / salt2fluxerr > 3]
                == np.max(salt2flux[salt2flux / salt2fluxerr > 3])
            ]
        )
        if len(pkguess):
            pkguess = pkguess[0]
            ...
            for bs, bn, b, bc, bsym, inn in zip(
                ...
                bandkey = "Band: %s - %s" % (inn, bn)
                if (
                    bandkey in bandpassdict.keys()
                    and bandpassdict[bandkey] in salt2band
                ):
                    salt2flux = fitted_model.bandflux(
...
def lightcurveplot_flux(request, transient_id, salt2=False):
    ...
    # ax=figure()
    mjd, salt2mjd, date, mag, magerr, magsys, flux, fluxerr, salt2flux, salt2fluxerr, zpsys, salt2band, band, bandstr, bandcolor, bandsym, data_quality = (
        np.array([]),
        np.array([]),
        np.array([]),
        np.array([]),
    ...
    for p in photdata:
        ...
        if p.flux or (not p.flux and p.mag):
            ...
            mag = np.append(mag, [p.mag])
            if p.mag_sys is None:
                magsys = np.append(magsys, ["None"])
            else:
                magsys = np.append(magsys, [p.mag_sys])
            if p.data_quality is None:
                data_quality = np.append(data_quality, ["Good"])
            else:
                data_quality = np.append(data_quality, [p.data_quality.name])
            ...
            if salt2:
                if str(p.band) in bandpassdict.keys():
                    ...
                    salt2fluxerr = np.append(
                        salt2fluxerr, flux_single * mag_err * 0.4 * np.log(10)
                    )
                    salt2band = np.append(salt2band, bandpassdict[str(p.band)])
                    if "bessell" in bandpassdict[str(p.band)]:
                        zpsys = np.append(zpsys, "Vega")
    ...
    for bs, b, bc, bsym in zip(
        ...
        p = plotmethod(
            "x", "y", source=source, color=color, size=size, muted_alpha=0.2
        )  # ,legend='%s - %s'%(
        ...
        for x, y, yerr in zip(
            mjd[bandstr == bs].tolist(),
            flux[bandstr == bs].tolist(),
            fluxerr[bandstr == bs].tolist(),
        ):
            err_xs.append((x, x))
            err_ys.append((y - yerr, y + yerr))
    ...
    if limmjd:
        ax.x_range = Range1d(limmjd, np.max(mjd) + 10)
        ax.y_range = Range1d(
            0 - np.max(flux[mjd > limmjd]) * 0.03, np.max(flux[mjd > limmjd]) * 1.1
        )
    ...
    if salt2:
        ...
        zp = np.array([27.5] * len(salt2band))
        data = Table(
            [salt2mjd, salt2band, salt2flux, salt2fluxerr, zp, zpsys],
            names=["mjd", "band", "flux", "fluxerr", "zp", "zpsys"],
            meta={"t0": salt2mjd[salt2flux == np.max(salt2flux)]},
        )
        ...
        result, fitted_model = sncosmo.fit_lc(
            ...
            bounds={
                "t0": (
                    min(np.atleast_1d(salt2mjd[salt2flux == np.max(salt2flux)])) - 10,
...
def spectrumplot(request, transient_id):
    ...
    ax = figure(
        ...
        y_range=(-0.15, len(dbspectra) + 0.15),
    )
    ...
    for i, color in zip(range(len(dbspectra)), colors):
        ...
        if len(spectra[i]["flux"]):
            p = ax.line(
                spectra[i]["wave"],
                (spectra[i]["flux"] - spectra[i].minval) / spectra[i].scale
                + spectra[i].offset,
                color=color,
                muted_alpha=0.2,
            )

        legend_it[np.where(temp2 == i)[0][0]] = (
            "%s - %s"
            % (
                dbspectra[i].instrument.name,
                dbspectra[i].obs_date.strftime("%Y-%m-%d"),
            ),
            [p],
        )
...
def spectrumplotsingle(request, transient_id, spec_id):
    ...
    for s in spec:
        wave += [s.wavelength]
        flux += [s.flux]
    bottom = np.percentile(flux, 5) * 0.5
    top = np.percentile(flux, 95) * 1.3
    if np.percentile(flux, 2) < 0:
        bottom = np.percentile(flux, 2) * 1.5
    ...
    ax.title.text = "%s, %s, %s, Phase: %s, DQ: %s" % (
        transient.name,
        spectrum.obs_date.strftime("%m/%d/%Y"),
        spectrum.instrument,
        spectrum.spec_phase,
        spectrum.data_quality,
    )
    ax.xaxis.axis_label = r"Wavelength (Angstrom)"
    ax.yaxis.axis_label = "Flux"
    g = file_html(ax, CDN, "my plot")
...
def rise_time(request, transient_id, obs_id):
    ...
    action = Process(
        target=rise_func,
        args=(coords, obs_date, longitude, latitude, elevation, risedict),
    )
    action.start()
...
def set_time(request, transient_id, obs_id):
    ...
    action = Process(
        target=set_func,
        args=(coords, obs_date, longitude, latitude, elevation, setdict),
    )
    action.start()
...
def get_ps1_image(request, transient_id):
    ...
    ps1url = (
        "http://plpsipp1v.stsci.edu/cgi-bin/ps1cutouts?pos=%.7f+%.7f&filter=color"
        % (t.ra, t.dec)
    )
    try:
    ...
    if "<td><img src=" in response.content.decode("utf-8"):
        jpegurl = (
            ...
            .split('<td><img src="')[1]
            .split('" width="240" height="240" /></td>')[0]
        )
...
def get_hst_image(request, transient_id):
    ...
    for jpg, i in zip(hst.jpglist, range(len(hst.jpglist))):
        print(jpg)
        fitsurllist += [
            "https://hla.stsci.edu/cgi-bin/getdata.cgi?config=ops&amp;dataset=%s"
            % str(hst.obstable["obs_id"][i]).lower()
        ]
    print(
        "Run time was: ",
    ...
    if len(hst.jpglist):
        jpegurldict = {
            ...
            "obsdate": list(
                Time(hst.obstable["t_min"], format="mjd").iso
            ),  # ,out_subfmt='date'
...
def get_chandra_image(request, transient_id):
    ...
    if len(chr.jpegs):
        jpegurldict = {
            "jpegurl": list(chr.jpegs.data.astype(str)),
...
def get_legacy_image(request, transient_id):
    ...
    jpegurl = (
        "http://legacysurvey.org/viewer/jpeg-cutout?ra=%.7f&dec=%.7f&pixscale=0.27&bands=grz"
        % (t.ra, t.dec)
    )
    ...
    jpegurldict = {"jpegurl": jpegurl, "fitsurl": fitsurl}
...
def get_authorized_classical_resources(user):
    allowed_resources = ObservingResourceService.GetAuthorizedClassicalResource_ByUser(
        user
    )
...
def get_authorized_queued_resources(user):
    allowed_resources = ObservingResourceService.GetAuthorizedQueuedResource_ByUser(
        user
    )
