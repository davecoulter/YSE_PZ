...
"""
    ...
    # g/V rise
    qs = (
        qs.annotate(recent_g_mag=RawSQL(last_mag_query % ("pd.mag", g_filters, 0), ()))
        .annotate(
            recent_g_mag_err=RawSQL(last_mag_query % ("pd.mag_err", g_filters, 0), ())
        )
        .annotate(
            recent_g_obs=RawSQL(
                last_mag_query % ("UNIX_TIMESTAMP(pd.obs_date)", g_filters, 0), ()
            )
        )
        .annotate(
            second_most_recent_g_mag=RawSQL(
                last_mag_query % ("pd.mag", g_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_g_obs=RawSQL(
                last_mag_query % ("UNIX_TIMESTAMP(pd.obs_date)", g_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_g_ulim=RawSQL(
                last_mag_query
                % ("-2.5*LOG10(pd.flux+3*pd.flux_err)+27.5 as lim", g_filters, 1),
                (),
            )
        )
        .annotate(
            second_most_recent_g_flux=RawSQL(
                last_mag_query % ("pd.flux", g_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_g_flux_err=RawSQL(
                last_mag_query % ("pd.flux_err", g_filters, 1), ()
            )
        )
    )
    ...
    qs = qs.annotate(
        rise_rate_g=ExpressionWrapper(
            F("dm_g") / (Greatest(F("dt_g"), 4.0 * 60.0 * 60.0) / 24.0 / 60.0 / 60.0),
            output_field=FloatField(),
        )
    )
    qs = qs.annotate(
        dm_lim_g=-1.0 * (F("recent_g_mag") - F("second_most_recent_g_ulim"))
    )
    qs = qs.annotate(
        rise_rate_g_lim=ExpressionWrapper(
            F("dm_lim_g") / (F("dt_g") / 24.0 / 60.0 / 60.0), output_field=FloatField()
        )
    )
    ...
    # r rise
    qs = (
        qs.annotate(recent_r_mag=RawSQL(last_mag_query % ("pd.mag", r_filters, 0), ()))
        .annotate(
            recent_r_mag_err=RawSQL(last_mag_query % ("pd.mag_err", r_filters, 0), ())
        )
        .annotate(
            recent_r_obs=RawSQL(
                last_mag_query % ("UNIX_TIMESTAMP(pd.obs_date)", r_filters, 0), ()
            )
        )
        .annotate(
            second_most_recent_r_mag=RawSQL(
                last_mag_query % ("pd.mag", r_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_r_obs=RawSQL(
                last_mag_query % ("UNIX_TIMESTAMP(pd.obs_date)", r_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_r_ulim=RawSQL(
                last_mag_query
                % ("-2.5*LOG10(pd.flux+3*pd.flux_err)+27.5 as lim", r_filters, 1),
                (),
            )
        )
        .annotate(
            second_most_recent_r_flux=RawSQL(
                last_mag_query % ("pd.flux", r_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_r_flux_err=RawSQL(
                last_mag_query % ("pd.flux_err", r_filters, 1), ()
            )
        )
    )
    ...
    qs = qs.annotate(
        rise_rate_r=ExpressionWrapper(
            F("dm_r") / (F("dt_r") / 24.0 / 60.0 / 60.0), output_field=FloatField()
        )
    )
    qs = qs.annotate(
        dm_lim_r=-1.0 * (F("recent_r_mag") - F("second_most_recent_r_ulim"))
    )
    qs = qs.annotate(
        rise_rate_r_lim=ExpressionWrapper(
            F("dm_lim_r") / (F("dt_r") / 24.0 / 60.0 / 60.0), output_field=FloatField()
        )
    )
    ...
    # i rise
    qs = (
        qs.annotate(recent_i_mag=RawSQL(last_mag_query % ("pd.mag", i_filters, 0), ()))
        .annotate(
            recent_i_mag_err=RawSQL(last_mag_query % ("pd.mag_err", i_filters, 0), ())
        )
        .annotate(
            recent_i_obs=RawSQL(
                last_mag_query % ("UNIX_TIMESTAMP(pd.obs_date)", i_filters, 0), ()
            )
        )
        .annotate(
            second_most_recent_i_mag=RawSQL(
                last_mag_query % ("pd.mag", i_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_i_obs=RawSQL(
                last_mag_query % ("UNIX_TIMESTAMP(pd.obs_date)", i_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_i_ulim=RawSQL(
                last_mag_query
                % ("-2.5*LOG10(pd.flux+3*pd.flux_err)+27.5 as lim", i_filters, 1),
                (),
            )
        )
        .annotate(
            second_most_recent_i_flux=RawSQL(
                last_mag_query % ("pd.flux", i_filters, 1), ()
            )
        )
        .annotate(
            second_most_recent_i_flux_err=RawSQL(
                last_mag_query % ("pd.flux_err", i_filters, 1), ()
            )
        )
    )
    ...
    qs = qs.annotate(
        rise_rate_i=ExpressionWrapper(
            F("dm_i") / (F("dt_i") / 24.0 / 60.0 / 60.0), output_field=FloatField()
        )
    )
    qs = qs.annotate(
        dm_lim_i=-1.0 * (F("recent_i_mag") - F("second_most_recent_i_ulim"))
    )
    qs = qs.annotate(
        rise_rate_i_lim=ExpressionWrapper(
            F("dm_lim_i") / (F("dt_i") / 24.0 / 60.0 / 60.0), output_field=FloatField()
        )
    )
...
def fastrising_transient_queryset(ndays=7):
    ...
    qs = Transient.objects.filter(
        created_date__gte=datetime.datetime.utcnow() - datetime.timedelta(ndays)
    )
    qs = annotate_rising_transient_qs(qs)

    qs_final = qs.filter(
        (Q(rise_rate_g__gte=0.5) & (Q(rise_rate_g_sig__gte=1)))
...
def rising_transient_queryset(ndays=14):
    ...
    qs = annotate_rising_transient_qs(qs)
    qs_final = qs.filter(
        (Q(rise_rate_g__gte=0) & (Q(rise_rate_g_sig__gte=1)))
...
def tns_yse_rising_transient_queryset(ndays=14):
    ...
    qs = (
        ...
        .filter(tags__name__in="YSE")
        .filter(name__startswith="20")
    )
    qs = annotate_rising_transient_qs(qs)
    qs_final = qs.filter(
        (Q(rise_rate_g__gte=0.0) & (Q(rise_rate_g_sig__gte=1.0)))
...
def sne_in_last_nights_fields():
    ...
    ).filter(tags__name="YSE")
    survey_obs = SurveyObservation.objects.filter(
        obs_mjd__gte=date_to_mjd(datetime.datetime.utcnow() - datetime.timedelta(1))
    )
    qs_list = []
    done_list = []
    for s in survey_obs:
        if s.survey_field.field_id in done_list:
            continue
        ...
        qs_list += [
            (
                Q(ra__gt=s.survey_field.ra_cen - ra_offset.degree)
...
def sne_15deg_from_yse(qs_start=None):
    ...
    survey_obs = SurveyObservation.objects.filter(
        Q(obs_mjd__gte=date_to_mjd(datetime.datetime.utcnow() - datetime.timedelta(14)))
        | Q(
            mjd_requested__gte=date_to_mjd(
...
""" % (
            ...
            s.survey_field.dec_cen,
            s.survey_field.ra_cen,
            s.survey_field.ra_cen - ra_offset.degree,
            s.survey_field.ra_cen + ra_offset.degree,
            s.survey_field.dec_cen - dec_offset.degree,
            s.survey_field.dec_cen + dec_offset.degree,
        )
        ...
        if qs_start is None:
            qs_single = (
                Transient.objects.filter(
                    created_date__gte=datetime.datetime.utcnow()
                    - datetime.timedelta(90)
                )
                .filter(~Q(tags__name="YSE"))
...
HAVING sep < 3.3
""" % (
        survey_field.dec_cen,
        survey_field.dec_cen,
        survey_field.ra_cen,
        survey_field.ra_cen - ra_offset.degree,
        survey_field.ra_cen + ra_offset.degree,
        survey_field.dec_cen - dec_offset.degree,
        survey_field.dec_cen + dec_offset.degree,
    )
    cursor = connections["explorer"].cursor()
    cursor.execute(query, ())
