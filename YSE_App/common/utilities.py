...
def GetSexigesimalString(ra_decimal, dec_decimal):
	...
	ra_string = "%02d:%02d:%04.1f" % (ra[0],ra[1],ra[2])
	if dec[0] >= 0:
		dec_string = "%02d:%02d:%04.1f" % (dec[0],np.abs(dec[1]),np.abs(dec[2]))
	else:
		dec_string = "%03d:%02d:%04.1f" % (dec[0],np.abs(dec[1]),np.abs(dec[2]))
	...
	if c.dec < 0.0 and dec[0] == 0.0:
		dec_string = "-00:%02d:%04.1f" % (np.abs(dec[1]),np.abs(dec[2]))
