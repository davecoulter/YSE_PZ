...
class AuditTrail(object):
    ...
    def contribute_to_class(self, cls, name):
        ...
        def _contribute(sender, **kwargs):
            ...
            descriptor = AuditTrailDescriptor(
                model._default_manager, sender._meta.pk.attname
            )
...
class AuditTrailDescriptor(object):
    ...
    def __get__(self, instance=None, owner=None):
        if instance == None:
            ...
            return create_audit_manager_with_pk(
                self.manager, self.pk_attribute, instance._get_pk_val()
            )
...
def create_audit_manager_with_pk(manager, pk_attribute, pk):
    ...
    class AuditTrailWithPkManager(manager.__class__):
        ...
        def get_query_set(self):
            qs = (
                super(AuditTrailWithPkManager, self)
                .get_query_set()
                .filter(**{pk_attribute: pk})
            )
...
def create_audit_model(cls, **kwargs):
    ...
    attrs = {
        ...
        % (self._audit__str__(), self._audit_timestamp),
        "_audit_track": _track_fields(
            track_fields=kwargs["track_fields"], unprocessed=True
        ),
    }
    ...
    for field in cls._meta.fields:
        ...
        if field.name in attrs:
            raise ImproperlyConfigured(
                "%s cannot use %s as it is needed by AuditTrail."
    ...
    for track_field in _track_fields(kwargs["track_fields"]):
        if track_field["name"] in attrs:
            raise NameError(
                'Field named "%s" already exists in audit version of %s'
                % (track_field["name"], cls.__name__)
            )
