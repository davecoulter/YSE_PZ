...
class hstImages:
    ...
    def getObstable(self):
        ...
        table = Observations.query_region(self.coord, radius=self.options["radius"])

        # HST-specific masks
        filmask = [table["filters"] == good for good in options["mask"]["filters"]]
    ...
    def getJPGurl(self):
        ...
        for obsid in self.obstable["obs_id"]:
            url = self.options["jpg"].format(
                id=obsid, ra=self.coord.ra.degree, dec=self.coord.dec.degree
            )
