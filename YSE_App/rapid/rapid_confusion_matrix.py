...
def main():
    ...
    ax = plt.axes()
    class_names = {
        "SN Ia": ["SN Ia", "SN Ia-91T-like", "SN Ia-91bg-like", "SN Ia-pec"],
        "SN Ib/c": [
            "SN Ic",
            "	SN Ib/c",
            "SN Ib-Ca-rich",
    ...
    all_labels = np.concatenate(
        (class_names["SN Ia"], class_names["SN Ib/c"], class_names["SN II"])
    )
    ...
    for i, class1 in enumerate(["SN Ia", "SN Ib/c", "SN II"]):
        for j, class2 in enumerate(["SN Ia", "SN Ib/c", "SN II"]):
            ...
            list_correct = len(
                transients_to_classify.filter(photo_class__name=class1).filter(
                    best_spec_class__name__in=class2list
                )
            )
            list_full = len(
                transients_to_classify.filter(
                    best_spec_class__name__in=class2list
                ).filter(photo_class__name__in=["SN Ia", "SN Ib/c", "SN II"])
            )

            matrix[j, i] = float(list_correct) / list_full

    cb = ax.imshow(
        matrix.transpose(), cmap=plt.cm.Blues, vmin=0.0, vmax=1.0
    )  # ,origin="lower")
    for i in range(3):
        for j in range(3):
